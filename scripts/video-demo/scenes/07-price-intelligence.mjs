...
export default async function priceIntelligence(scene) {
  await scene.beat(
    "second-version",
    "Giulia apre la seconda versione del listino Alfa Medical.",
    "Il documento viene confrontato con la versione precedente sul prezzo normalizzato.",
    async () => {
      await scene.switchPersona(PERSONAS.giulia);
      await scene.goto("/imports");
      const href = await scene.page
        .getByRole("link", { name: "listino-alfa-medical-2028.xlsx" })
        .first()
        .getAttribute("href");
  ...
  await scene.beat(
    "increases",
    "Il filtro Aumenti isola le variazioni commerciali rilevanti.",
    "Le priorità emergono senza scorrere l'intero listino.",
    async () => {
  ...
  await scene.beat(
    "increase-detail",
    "Un aumento mostra precedente, nuovo e migliore corrente.",
    "La posizione del nuovo prezzo è leggibile rispetto al mercato interno disponibile.",
    async () => {
  ...
  await scene.beat(
    "packaging-change",
    "Un cambio confezione viene letto sul costo per unità di consumo.",
    "Il sistema non scambia l'aumento del prezzo scatola con un aumento reale per pezzo.",
    async () => {
  ...
  await scene.beat(
    "old-new-summary",
    "La sintesi old-versus-new separa aumenti, riduzioni, nuovi e rimossi.",
    "Un documento commerciale diventa una mappa di decisioni procurement.",
    async () => {
