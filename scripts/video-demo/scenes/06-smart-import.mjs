...
export default async function smartImport(scene) {
  await scene.beat(
    "import-work-queue",
    "Giulia apre la coda delle importazioni da gestire.",
    "Gli import che richiedono una decisione vengono prima dello storico.",
    async () => {
  ...
  await scene.beat(
    "exceptions-first",
    "Un documento sporco mostra poche eccezioni rispetto alle righe interpretate.",
    "Il valore è lavorare sulle eccezioni, non leggere l'intero documento.",
    async () => {
  ...
  await scene.beat(
    "uncertain-record",
    "La coda espone subito i record incerti e la loro causa.",
    "Giulia non deve leggere le righe affidabili: match, unità e confezioni ambigue arrivano prima.",
    async () => {
  ...
  await scene.beat(
    "upload-real-xlsx",
    "Giulia carica un file XLSX reale e seleziona il fornitore.",
    "Il file originale viene conservato; in questo ambiente lavora l'interpretazione locale.",
    async () => {
      ...
      await scene.select(scene.page.getByRole("combobox", { name: /^Fornitore/ }), {
        label: "Alfa Medical",
      });
      ...
      await scene.page.waitForURL((url) => /^\/imports\/(?!new(?:\/|$))[^/]+$/.test(url.pathname), {
        timeout: 60_000,
      });
      await scene.assertText("Revisione per eccezione");
    },
  ...
  await scene.beat(
    "column-mapping",
    "Il mapping colonne mostra cosa è stato riconosciuto automaticamente.",
    "Parser deterministico e mapping euristico restano modificabili prima della revisione.",
    async () => {
  ...
  await scene.beat(
    "interpreted-record",
    "Una riga pronta mostra confezione e prezzo normalizzato coerenti.",
    "Una scatola da cento a due euro e cinquanta diventa zero virgola zero due cinque per pezzo.",
    async () => {
  ...
  await scene.beat(
    "matching-reasons",
    "Il match consigliato espone segnali e differenze.",
    "Il punteggio non è decorativo: la UI spiega perché il candidato è proposto.",
    async () => {
  ...
  await scene.beat(
    "provenance",
    "La provenienza si espande solo quando serve.",
    "Ogni campo conserva valore grezzo, interpretazione, normalizzazione e posizione sorgente.",
    async () => {
  ...
  await scene.beat(
    "confirm-safe-batch",
    "Le proposte ad alta affidabilità vengono confermate insieme.",
    "Il controllo umano resta esplicito, ma non costringe a ripetere decisioni identiche.",
    async () => {
      await scene.goto(importPath);
      const button = scene.page.getByRole("button", {
        name: "Conferma tutte le proposte affidabili",
      });
  ...
  await scene.beat(
    "publish-gate",
    "Il riepilogo rende espliciti record pronti e blocchi residui.",
    "Nessun dato entra nel catalogo prima del gate di pubblicazione.",
    async () => {
  ...
  await scene.beat(
    "publish",
    "Giulia conferma la nuova versione del listino.",
    "La pubblicazione è atomica, idempotente e conserva il collegamento alla fonte.",
    async () => {
      await scene.click(scene.page.getByRole("button", { name: "Pubblica importazione" }), {
        settle: false,
      });
