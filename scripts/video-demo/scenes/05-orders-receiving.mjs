...
export default async function ordersReceiving(scene) {
  await scene.beat(
    "orders",
    "Lucia apre gli ordini della struttura.",
    "Richiesta e decisione continuano in un oggetto operativo distinto.",
    async () => {
  ...
  await scene.beat(
    "po-detail",
    "Il dettaglio ordine riunisce righe, consegna, condizioni e origine.",
    "Ogni quantità resta riconciliata con ricezioni e problemi.",
    async () => {
  ...
  await scene.beat(
    "delivery-board",
    "La bacheca consegne separa ritardi, oggi, prossime e ricevute.",
    "La struttura vede subito cosa richiede un'azione.",
    async () => {
  ...
  await scene.beat(
    "receive",
    "Lucia apre una ricezione ancora disponibile.",
    "Il flusso parte dalle quantità residue reali.",
    async () => {
  ...
  await scene.beat(
    "partial-with-issue",
    "Viene registrata una consegna parziale con una non conformità.",
    "Quantità ricevuta, accettata e problematica restano tracciate sulla riga.",
    async () => {
      ...
      await row
        .locator('input[name^="issueNote-"]')
        .fill("Una confezione presenta un danno visibile al ricevimento.");
