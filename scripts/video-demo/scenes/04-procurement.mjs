...
export default async function procurement(scene) {
  await scene.beat(
    "control-center",
    "Giulia entra nel Centro di controllo Procurement.",
    "La pagina parte dalle eccezioni che richiedono attenzione.",
    async () => {
  ...
  await scene.beat(
    "actionable-priority",
    "Una priorità mostra gravità, anzianità e contesto.",
    "La coda indica dove intervenire prima, senza cercare tra moduli separati.",
    async () => {
      const queue = scene.page.locator(".procurement-queue, .attention-list").first();
      if (await queue.count()) await scene.focus(queue, 1900);
      else
        await scene.focus(
          scene.page.getByText("Cosa richiede attenzione", { exact: false }).first(),
        );
    },
    "analysis",
  );
  await scene.beat(
    "supplier-directory",
    "Giulia apre il portafoglio fornitori.",
    "Ogni fornitore collega spesa, offerta, consegne e qualità.",
    async () => {
  ...
  await scene.beat(
    ...
    "Dipendenza e performance rendono leggibile il rischio commerciale.",
    "Quota spesa, copertura e campione osservato evitano score opachi.",
    async () => {
  ...
  await scene.beat(
    "category-360",
    "La lettura passa dalla singola controparte alla categoria.",
    "Categoria 360 unisce domanda, concentrazione e segnali di prezzo.",
    async () => {
