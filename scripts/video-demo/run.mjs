...
function requestedScene() {
  ...
  if (!scene)
    throw new Error(
      `Scena sconosciuta "${value}". Valori: ${SCENES.map((item) => item.aliases[0]).join(", ")}`,
    );
  return scene;
}
...
async function recordScene(browser, definition) {
  ...
  const context = await browser.newContext({
    ...
    recordVideo: { dir: tempDirectory, size: VIEWPORT },
  });
  ...
  try {
    ...
    if (scene.consoleErrors.length)
      throw new Error(`Errori browser: ${scene.consoleErrors.join(" | ")}`);
    await scene.writeOutputs({ browser: "Chromium Playwright", viewport: VIEWPORT });
  } catch (error) {
    failure = error;
    await page
      .screenshot({ path: `${PATHS.reports}/${definition.id}-failure.png`, fullPage: false })
      .catch(() => {});
  } finally {
    await context.close();
  }
  ...
  const destination = failure
    ? `${PATHS.reports}/${definition.id}-failed.webm`
    : `${PATHS.clips}/${definition.id}.webm`;
...
export async function runVideoDemo() {
  ...
  if (!single) await cleanVideoArtifacts();
  else await ensureDirectories();
  ...
  try {
    ...
    for (const definition of selected) {
      ...
      try {
        results.push(await recordScene(browser, definition));
        console.log("PASS");
      } catch (error) {
        results.push({
          scene: definition.id,
    ...
    const durations = Object.fromEntries(
      validation.results.map((item) => [item.scene, item.durationSeconds]),
    );
    for (const definition of selected) {
    ...
    const report = {
      ...
      mode: single ? "single-scene" : "complete",
      scenes: results,
