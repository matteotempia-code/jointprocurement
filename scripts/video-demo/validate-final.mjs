...
const required = [
  "joint-procurement-demo-v1.mp4",
  "joint-procurement-demo-v1-subtitled.mp4",
  "joint-procurement-demo-it.srt",
  "joint-procurement-demo-it.vtt",
];
for (const name of required) {
  const file = path.join(finalDir, name);
  if (!fs.existsSync(file) || fs.statSync(file).size < (name.endsWith(".mp4") ? 1_000_000 : 50))
    throw new Error(`Invalid final artifact: ${name}`);
}
...
for (const name of required.filter((x) => x.endsWith(".mp4"))) {
  ...
  if (
    ...
    errors.push("video codec/geometry/pixel format");
  if (audio?.codec_name !== "aac" || Number(audio.sample_rate) !== 48000)
    errors.push("audio codec/sample rate");
  if (!audio || !video) errors.push("missing stream");
  if (Number(data.format.duration) < 780 || Number(data.format.duration) > 920)
    errors.push("duration outside 13–15 minute target");
...
for (const scene of report.scenes) {
  points.push(cursor + Math.min(4, scene.finalSceneDuration / 2));
  cursor += scene.finalSceneDuration;
}
...
points.forEach((at, index) => {
  const output = path.join(
    framesDir,
    `${String(index).padStart(2, "0")}-${at.toFixed(2).replace(".", "_")}s.png`,
  );
  execFileSync(ffmpegPath, ["-y", "-ss", at.toFixed(3), "-i", master, "-frames:v", "1", output], {
    stdio: "ignore",
  });
  if (fs.statSync(output).size < 10_000) throw new Error(`Invalid sampled frame at ${at}s`);
  validation.sampleFrames.push({
    at,
...
fs.writeFileSync(
  path.join(finalDir, "validation-report.json"),
