...
export async function validateVideos(sceneDefinitions = SCENES) {
  ...
  await writeFile(
    playerPath,
    "<!doctype html><html><body style='margin:0;background:#111'><video id='v' muted playsinline></video><canvas id='c' width='160' height='90'></canvas></body></html>",
    "utf8",
  );
    ...
    for (const definition of sceneDefinitions) {
      ...
      const decoded = await page.evaluate(async (source) => {
        ...
        for (const ratio of [0.18, 0.5, 0.82]) {
          const time = Math.max(0, Math.min(video.duration - 0.05, video.duration * ratio));
      ...
      const manifest = JSON.parse(
        await readFile(`${PATHS.manifests}/${definition.id}.json`, "utf8"),
      );
      const pass =
        info.size > 350_000 &&
        decoded.durationSeconds > 5 &&
        decoded.width === VIEWPORT.width &&
        decoded.height === VIEWPORT.height &&
        decoded.frames.every((frame) => frame.nonBlack) &&
        manifest.visualBeats.length > 0 &&
      ...
      results.push({
        ...
        bytes: info.size,
        ...decoded,
        expectedPageVerifiedBy: `screenshots/${definition.id}.png`,
  ...
  const report = {
    ...
    method: "Decodifica Chromium WebM + campionamento luminanza/varianza al 18%, 50% e 82%",
    ffprobeAvailable: false,
    overall: results.every((item) => item.status === "PASS") ? "PASS" : "FAIL",
    totalDurationSeconds: Number(
      results.reduce((sum, item) => sum + item.durationSeconds, 0).toFixed(3),
    ),
    results,
  };
  await writeFile(
    `${PATHS.reports}/technical-validation.json`,
    JSON.stringify(report, null, 2),
    "utf8",
  );
  if (report.overall !== "PASS")
    throw new Error(
      `Validazione tecnica video fallita: ${results
