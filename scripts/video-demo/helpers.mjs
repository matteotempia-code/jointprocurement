...
const cursorInit = () => {
  const install = () => {
    ...
    Object.assign(cursor.style, {
      ...
      width: "13px",
      height: "13px",
      border: "1.5px solid rgba(28,25,21,.88)",
      borderRadius: "50%",
      background: "rgba(255,255,255,.82)",
      boxShadow: "0 2px 7px rgba(28,25,21,.2)",
      pointerEvents: "none",
      opacity: ".34",
      transform: "translate(-50%,-50%)",
      transition:
        "left .42s cubic-bezier(.2,.75,.25,1), top .42s cubic-bezier(.2,.75,.25,1), opacity .18s, transform .18s",
    });
    document.documentElement.append(cursor);
    window.__jpVideoCursor = {
      move(x, y) {
        cursor.style.opacity = ".92";
        cursor.style.left = `${x}px`;
        cursor.style.top = `${y}px`;
        cursor.style.transform = "translate(-50%,-50%) scale(1)";
        clearTimeout(window.__jpCursorIdle);
        window.__jpCursorIdle = setTimeout(() => {
...
export class DemoScene {
  ...
  async pause(kind = "read") {
    ...
    await this.page.waitForTimeout(
      Math.max(120, Math.round((PAUSES[kind] ?? Number(kind) ?? PAUSES.read) * scale)),
    );
  }
  async settle() {
  ...
  async goto(path, options = {}) {
    const response = await this.page.goto(new URL(path, BASE_URL).toString(), {
...
export async function installDemoCursor(context) {
  await context.addInitScript(cursorInit);
}
export function selectorSlug(value) {
  return slug(value);
}
