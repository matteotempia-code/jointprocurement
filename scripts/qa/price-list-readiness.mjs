...
if (!(await ready())) {
  ...
  for (let attempt = 0; attempt < 120 && !(await ready()); attempt += 1)
...
async function switchTo(name) {
  await open("/");
  const selector = page.getByLabel(/^(Persona demo|Visualizza come)$/);
  ...
  if (!value) throw new Error(`Persona non trovata: ${name}`);
...
async function uploadAndPublish({ fixture, supplier, label, existingId }) {
  if (existingId) await open(`/imports/${existingId}`);
    ...
    if (!(await page.getByTestId("import-file").count()))
      throw new Error(`Upload non disponibile: ${await page.locator("main").innerText()}`);
    await page
      .getByTestId("import-file")
      .setInputFiles(path.join(process.cwd(), "demo-imports", fixture));
  ...
  const confirmation = page.getByRole("heading", {
    name: "A quale fornitore appartiene il documento?",
  });
  ...
  for (let pass = 0; pass < 1; pass += 1) {
    const attention = page.locator('input[name="recordId"]');
    ...
    await Promise.all([
      page.waitForURL(/batch=/),
      page.locator(".bulk-review-bar").getByRole("button", { name: "Applica" }).click(),
    ]);
    await open(`/imports/${jobId}?filtro=attention`);
  }
  for (let decision = 0; decision < 100; decision += 1) {
    ...
    const recordHref = await page
      .locator('table tbody a[href*="/records/"]')
      .first()
      .getAttribute("href");
    if (!recordHref) break;
    ...
    if (!(await confirm.count()))
      throw new Error(`${label}: record incerto senza candidato confermabile (${recordHref}).`);
    await confirm.click();
    await page.waitForLoadState("networkidle");
  }
  ...
  if (await approve.count())
    await Promise.all([page.waitForURL(/alta=approvata/), approve.click()]);
  ...
  if (await publish.count()) {
    await publish.click();
    await page.getByRole("dialog").getByRole("button", { name: "Pubblica", exact: true }).click();
    await page.waitForLoadState("networkidle");
    await open(`/imports/${jobId}/summary`);
  }
  await page
    .getByText(/offerte pubblicate/i)
    .first()
    .waitFor();
...
try {
  ...
  const alfa = await uploadAndPublish({
    fixture: "listino-alfa-medical-2027.xlsx",
    supplier: "Alfa Medical",
    label: "01-alfa-xlsx",
    existingId: resume[0],
  });
  const care = await uploadAndPublish({
    fixture: "offerta-caresupply-sporca.csv",
    supplier: "CareSupply",
    label: "02-caresupply-csv",
    existingId: resume[1],
  });
  ...
  const product = await page
    .locator('#opportunita a[href^="/products/"]')
    .first()
    .getAttribute("href");
  if (!product) throw new Error("Nessun prodotto confrontabile dalla categoria.");
  await open(product);
  const rows = await page
    .getByRole("table", { name: "Confronto offerte fornitori" })
    .locator("tbody tr")
    .count();
