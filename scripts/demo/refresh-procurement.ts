...
function plan(refs: Awaited<ReturnType<typeof references>>) {
  ...
  for (let i = 0; i < 520; i += 1) {
    ...
    audits.push({ id: id("audit-pr", i), organizationId: facility.area.legalEntity.organizationId, actorUserId: requester.id, entityType: "PURCHASE_REQUISITION", entityId: reqId, action: "REQUISITION_CREATED", metadata: { seeded: true, demoDataset: MARKER }, createdAt });
    ...
    audits.push({ id: id("audit-po", i), organizationId: facility.area.legalEntity.organizationId, actorUserId: refs.areaAssignment.userId, entityType: "PURCHASE_ORDER", entityId: poId, action: "PO_CREATED", metadata: { seeded: true, demoDataset: MARKER }, createdAt: issuedAt });
