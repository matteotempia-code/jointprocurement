...
async function main() {
  const groupedSources = await prisma.sourceDocument.groupBy({
    by: ["storageProvider"],
    _count: true,
  });
  const groupedAttachments = await prisma.operationalAttachment.groupBy({
    by: ["kind", "storageProvider"],
    _count: true,
  });
  const lucia = await prisma.user.findUnique({
    where: { email: "lucia.ferri@demo.local" },
    include: { carts: { include: { lines: true } } },
  });
  const counts = {
  ...
  const failures = [
    ...
    groupedSources.some((item) => item.storageProvider !== "supabase") &&
      "SourceDocument non cloud",
  ].filter(Boolean);
  ...
  console.log(
    JSON.stringify(
      {
        counts,
        sourceDocuments: groupedSources,
        operationalAttachments: groupedAttachments,
        luciaCartLines: lucia?.carts.reduce((sum, cart) => sum + cart.lines.length, 0) ?? 0,
      },
      null,
      2,
    ),
  ...
  console.log(
    failures.length ? `M11 READINESS: FAIL — ${failures.join(", ")}` : "M11 READINESS: READY",
  );
...
main()
  .catch((error) => {
    console.error(error instanceof Error ? error.message : "Verifica M11 fallita.");
    process.exitCode = 1;
  })
