...
function probableSource(key: string) {
  ...
  if (
    key.includes("/source-documents/") ||
    (key.includes("/imports/") && key.includes("/documents/"))
  )
    return "Smart Import source document";
...
async function main() {
  ...
  const [objects, sources, attachments] = await Promise.all([
    ...
    prisma.sourceDocument.findMany({
      where: { storageProvider: "supabase", storageBucket: bucket },
      select: { id: true, storageObjectKey: true, checksum: true },
    }),
  ...
  for (const item of sources)
    if (item.storageObjectKey)
      references.set(item.storageObjectKey, {
  ...
  const report = objects.map((object) => {
    ...
    return {
      ...
      classification: reference
        ? "REFERENCED"
        : probableSource(object.name) === "unknown"
          ? "UNKNOWN"
          : "ORPHANED",
    };
  });
  const objectNames = new Set(objects.map((item) => item.name));
  const missingObjects = [...references.entries()]
    .filter(([key]) => !objectNames.has(key))
    .map(([objectKey, reference]) => ({ objectKey, ...reference }));
  console.log(
    JSON.stringify(
      {
        bucket,
        objectCount: objects.length,
        referenced: report.filter((item) => item.classification === "REFERENCED").length,
        orphaned: report.filter((item) => item.classification === "ORPHANED").length,
        unknown: report.filter((item) => item.classification === "UNKNOWN").length,
        missingObjects,
        objects: report,
      },
      null,
...
main()
  .catch((error) => {
    console.error(error instanceof Error ? error.message : "Riconciliazione fallita.");
    process.exitCode = 1;
  })
