...
function resolveLegacyFile(storagePath: string) {
  ...
  const objectKey = normalized.startsWith("var/imports/")
    ? normalized.slice("var/imports/".length)
    : normalized;
  try {
...
async function main() {
  if (process.env.DOCUMENT_STORAGE_PROVIDER !== "supabase")
    throw new Error("DOCUMENT_STORAGE_PROVIDER deve essere 'supabase'.");
  const storage = new SupabaseDocumentStorage();
  const sources = await prisma.sourceDocument.findMany({
    where: { NOT: { storageProvider: "supabase" } },
  ...
  for (const source of sources) {
    ...
    if (!legacyFile) {
      missing += 1;
      console.log(`SKIP ${source.id}: locator locale non migrabile.`);
      continue;
    }
    let data: Buffer;
    try {
      data = await readFile(legacyFile);
    } catch {
      missing += 1;
      console.log(`SKIP ${source.id}: file locale assente.`);
      continue;
    }
    ...
    if (await storage.exists(locator)) {
      ...
      if (!existing.equals(data))
        throw new Error(`Oggetto esistente con contenuto differente per ${source.id}.`);
    } else {
        ...
        data: {
          ...
          metadata: {
            ...
            !Array.isArray(source.metadata)
              ? source.metadata
              : {}),
            migratedFrom: source.storagePath,
  ...
  console.log(
    `STORAGE MIGRATION: ${migrated} migrati, ${missing} saltati, ${sources.length} candidati.`,
  );
...
main()
  ...
  .catch((error) => {
    console.error(
      `STORAGE MIGRATION FAIL: ${error instanceof Error ? error.message : "errore sconosciuto"}`,
    );
    process.exitCode = 1;
  });
