...
async function main() {
  ...
  if (current?.public)
    throw new Error(
      `Il bucket '${storage.bucket}' esiste ma e pubblico. Renderlo privato prima di continuare.`,
    );
  if (!current) {
...
main().catch((error) => {
  console.error(
    `STORAGE SETUP FAIL: ${error instanceof Error ? error.message : "errore sconosciuto"}`,
  );
  process.exitCode = 1;
});
