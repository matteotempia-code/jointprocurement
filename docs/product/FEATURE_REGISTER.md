...
Ultima revisione: **2026-09-24**, dopo la certificazione cloud del ciclo P0.
...
>
> **Come leggere le revisioni.** Le righe marcate *audit 21/09/2026* portano un riferimento
> `file:riga`; quelle marcate *verificato 24/09/2026* sono state controllate sul codice allo
> SHA `a4d062b` dopo il PASS della pipeline 35887164989, **non trascritte da un resoconto**.
> Ogni percentuale è verificabile aprendo il file citato: se la verifica non regge, la riga
> va discussa e corretta, non accettata. Le righe non marcate mantengono la valutazione
> precedente e non sono state rimesse in discussione.
...
|---|---|---|---|---:|---|
| PLT-00 | **Autenticazione** | Verifica dell'identità di chi accede: password, sessione firmata, protezione delle route. | Core | **85%** | Implementata con Supabase Auth e certificata sul cloud. `src/proxy.ts` (il middleware, rinominato in Next 16) protegge tutte le route tranne `/login`, `/auth` e l'endpoint di salute, e **fallisce chiuso**: su eccezione redirige al login. Il cookie demo non contiene più l'id in chiaro — è cifrato in **AES-256-GCM** con tag di autenticazione e verifica di round-trip base64url (`src/lib/demo-session.ts`), e `openDemoUserId` **lancia** se `DEMO_MODE` non è `"true"`. Non al 100% per due ragioni: con `DEMO_MODE=true` il middleware lascia passare tutto senza autenticazione (accettabile solo in ambienti dimostrativi, mai con dati reali), e `DEMO_SESSION_SECRET` ricade su `SUPABASE_SERVICE_ROLE_KEY` se assente, riusando una chiave ad alto privilegio per cifrare un cookie. Identità enterprise: vedi PLT-11/12/13. — *verificato 24/09/2026* |
| PLT-01 | Multi-tenant | Separazione di più clienti/organizzazioni sulla stessa piattaforma. | Core pre-M11 | **75%** | `organizationId` aggiunta e verificata su tutte e sei le tabelle che ne erano prive — `Supplier`, `Category`, `CanonicalProduct`, `PriceList`, `SupplierOffer`, `AuditEvent` — con migrazione `20260922120000_tenant_catalog_isolation`. Control Tower ora filtra per organizzazione. Isolamento certificato sul cloud con fixture tenant-scoped. **Non al 100%**: la difesa resta interamente applicativa, RLS non è ancora attiva (vedi PLT-14). Un `where` dimenticato è ancora sufficiente a esporre dati fra tenant. — *verificato 24/09/2026* |
...
| ORD-02 | Numerazione PO | Identificativo univoco dell'ordine. | Core | **85%** | Sostituito il `count()+1` con una tabella contatore `DocumentSequence`, globale e con reset annuale; una migrazione successiva le rimuove `organizationId` rendendo le serie uniche di piattaforma. **Non al 100%**: è una tabella contatore, non una `SEQUENCE` nativa Postgres, quindi la correttezza sotto concorrenza dipende dall'atomicità dell'aggiornamento e non dal motore. Da verificare con un test di concorrenza reale. — *verificato 24/09/2026* |
...
| TECH-16 | Equivalenza funzionale | Confronta attributi critici di categoria senza affidarsi alla sola similarità testuale. | M12 | **85%** | **Comportamento predefinito invertito e certificato sul cloud.** Introdotta `compareTechnicalProfilesWithRules(a, b, criticalKeys, hasCategoryRules)` in `engine.ts:54`: se la categoria non ha requisiti configurati restituisce `INSUFFICIENT_EVIDENCE` con confidenza 0. Il vecchio comportamento fail-open non è raggiungibile per sbaglio, perché il parametro è esplicito e obbligatorio. — *verificato 24/09/2026* |
...
| SEC-01 | Cifratura connessione database | TLS verificato verso PostgreSQL. | P0 | **100%** | Chiusa. Certificato CA versionato nel repository (`certificates/supabase-root-2021-ca.crt`) e letto da `src/lib/database-tls.ts`, che rifiuta `sslmode=no-verify` e mantiene `rejectUnauthorized: true` anche in assenza del CA — fallisce chiuso. Nessun `NODE_TLS_REJECT_UNAUTHORIZED`. Verificato su tre run consecutivi. Il certificato è pubblico: versionarlo elimina un elemento dalla catena di configurazione da mantenere allineata. — *verificato 24/09/2026* |
