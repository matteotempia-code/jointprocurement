...
async function fixture(vatNumber) {
  ...
  sheet.addRow([
    "Ordine minimo: 150 EUR; franco porto: 500 EUR; trasporto: 18 EUR; consegna: 4 giorni",
  ]);
  sheet.addRow([]);
  ...
  sheet.addRow([
    "AI-CERT-AMB",
    "Detergente professionale concentrato formato da verificare",
    "",
    "",
    "",
    12.4,
  ...
  return {
    name: "certificazione-procurement-ai.xlsx",
...
try {
  ...
  const supplier = await db.supplier.findFirst({
    where: { name: { contains: "Alfa Medical", mode: "insensitive" } },
  ...
  await assert.doesNotReject(() =>
    runtimeStatus.getByText("AI status: OPENAI", { exact: true }).waitFor(),
  );
  ...
  const transientTimeout =
    job?.interpretationProvider !== "OPENAI" &&
    job?.procurementAICalls.some((call) => call.errorCode === "PROVIDER_TIMEOUT");
  if (transientTimeout) {
  ...
  assert.ok(
    job.procurementAICalls.some(
      (call) => call.operation === "DOCUMENT_CONTEXT" && call.resultState === "SUCCEEDED",
    ),
    "successful document-context call",
  );
  assert.ok(
    job.procurementAICalls.some((call) => call.operation === "ROW_INTERPRETATION"),
    "ambiguous row reached Procurement AI",
  );
  assert.ok(
    job.procurementAICalls.every(
      (call) => call.provider === "OPENAI" && call.model === displayedModel,
    ),
    "provider/model telemetry matches runtime UI",
  );

  const summary = job.summary ?? {};
  assert.ok(summary && typeof summary === "object" && !Array.isArray(summary));
  assert.ok(
    summary.aiSupplierSuggestion && typeof summary.aiSupplierSuggestion === "object",
    "AI supplier proposal persisted for review",
  );
  assert.ok(
    Array.isArray(summary.aiCommercialConditions) && summary.aiCommercialConditions.length > 0,
    "AI commercial conditions persisted for review",
  );
  ...
  console.log(
    JSON.stringify({
      ...
      successfulCalls: job.procurementAICalls.filter((call) => call.resultState === "SUCCEEDED")
        .length,
  ...
  const runtimeStatus = await page
    .locator(".provider-runtime-status")
    .innerText()
    .catch(() => "status unavailable");
  const safeStatus = runtimeStatus
    .replace(/\s+/g, " ")
  ...
  const safeMessage = (error instanceof Error ? error.message : String(error))
    ...
    .replace(/\s+/g, " ")
    .slice(0, 400);
  const jobDiagnostic = createdJobId
    ? await db.importJob
        .findUnique({
  ...
  if (process.env.GITHUB_ACTIONS === "true") {
    console.error(
      `::error title=Procurement AI remote ${checkpoint}::${safeMessage} | UI: ${safeStatus} | DB: ${JSON.stringify(jobDiagnostic)}`,
    );
  }
  throw error;
} finally {
