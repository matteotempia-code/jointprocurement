...
test("staging non muta il catalogo e publish è atomico, idempotente e conserva la provenienza", async () => {
  ...
  const supplier = await prisma.supplier.create({
    data: {
      ...
      name: "Fornitore Publish Test",
      vatNumber: `ITQA${Date.now()}`,
      active: true,
    },
  });
  const source = await prisma.sourceDocument.create({
    data: {
      ...
      originalFilename: "listino-publish-test.xlsx",
      mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      fileSize: 1200,
      checksum: `publish-test-${Date.now()}`,
      sourceType: "XLSX",
      documentKind: "PRICE_LIST",
      storagePath: "var/imports/test/listino-publish-test.xlsx",
      status: "PROCESSED",
    },
  });
  ...
  const record = await prisma.importedRecord.create({
    data: {
      importJobId: job.id,
      recordIndex: 1,
      rawSource: "QA-1;Prodotto;BOX;100;2,50",
      rawFields: { sku: "QA-1", price: "2,50" },
      interpretedFields: {
        supplierSku: "QA-1",
      ...
      normalizedFields: {
        ...
        consumptionUom: "PIECE",
        netPrice: 2.5,
        normalizedPrice: 0.025,
        normalizedLabel: "0,0250 € / pezzo",
        comparable: true,
        validationErrors: [],
        warnings: [],
        currency: "EUR",
      ...
      sourceLocator: { sheet: "Listino", row: 4, columns: { netPrice: "Prezzo netto" } },
  ...
  assert.equal(
    await prisma.canonicalProduct.count(),
    productsBefore,
    "lo staging non crea prodotti canonici",
  );
  try {
    ...
    const offer = await prisma.supplierOffer.findUniqueOrThrow({
    ...
    assert.equal(
      await prisma.auditEvent.count({
        where: { entityType: "IMPORT_JOB", entityId: job.id, action: "IMPORT_PUBLISHED" },
      }),
      1,
    );
  } finally {
    await prisma.offerPriceHistory.deleteMany({
      where: { supplierOffer: { importedRecordId: record.id } },
    });
      ...
      where: {
        OR: [
          { entityType: "IMPORT_JOB", entityId: job.id },
          {
            ...
            entityId: product.id,
            metadata: { path: ["importedRecordId"], equals: record.id },
          },
        ],
      },
    });
