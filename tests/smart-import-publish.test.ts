...
test("staging non muta il catalogo e publish è atomico, idempotente e conserva la provenienza", async () => {
  ...
  const product = await prisma.canonicalProduct.findFirstOrThrow({ where: { organizationId: organization.id }, orderBy: { name: "asc" } });
  const productsBefore = await prisma.canonicalProduct.count({ where: { organizationId: organization.id } });
  const supplier = await prisma.supplier.create({ data: { organizationId: organization.id, name: "Fornitore Publish Test", vatNumber: `ITQA${Date.now()}`, active: true } });
