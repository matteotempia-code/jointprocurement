...
test("object keys are organization-scoped, deterministic and traversal-safe", () => {
  ...
  const key = buildDocumentObjectKey({
    ...
    checksum,
    filename: "Listino Demo.xlsx",
  });
  assert.equal(
    key,
    `organizations/org_123/imports/doc-123/documents/doc-123/${checksum}-Listino-Demo.xlsx`,
  );
  ...
  const attachmentKey = buildOperationalAttachmentKey({
    organizationId: "org_123",
  ...
  assert.equal(
    attachmentKey,
    `organizations/org_123/procurement/quality-issue/issue_123/attachment_123/${checksum}-Foto-danno.jpg`,
  );
  assert.throws(() =>
    buildOperationalAttachmentKey({
      organizationId: "org_123",
...
test("local adapter supports immutable round-trip, metadata, deletion and missing objects", async () => {
  ...
  const locator: DocumentStorageLocator = {
    provider: "local",
    bucket: null,
    objectKey: "organizations/org/imports/doc/documents/doc/test.csv",
  };
  ...
  try {
    ...
    await assert.rejects(
      () => storage.put(locator, Buffer.from("different"), "text/csv"),
      /Collisione/,
    );
  ...
  try {
    for (let index = 0; index < 2; index += 1) {
      const jobId = await ingestDocument({
        buffer,
        filename: "storage-duplicate-test.csv",
        mimeType: "text/csv",
        supplierId: supplier.id,
        documentKind: "PRICE_LIST",
        organizationId: user.assignments[0].organizationId,
        userId: user.id,
      });
      ...
      if (index === 1)
        assert.equal((job.sourceDocument.metadata as { duplicateOf?: string }).duplicateOf, ids[0]);
    }
  } finally {
...
test("operational ingestion preserves CSV, XLSX, PDF and DOCX source bytes", async () => {
  ...
  zip.addFile(
    "word/document.xml",
    Buffer.from(
      '<?xml version="1.0"?><w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:r><w:t>sku;descrizione;prezzo;pezzi</w:t></w:r></w:p><w:p><w:r><w:t>CLOUD-DOCX;Guanto nitrile senza polvere M;2,50;100</w:t></w:r></w:p></w:body></w:document>',
    ),
  );
  const fixtureRoot = path.join(process.cwd(), "demo-imports");
  const cases = [
    {
      filename: "storage-e2e.csv",
      mimeType: "text/csv",
      buffer: Buffer.from(
    ...
    {
      filename: "storage-e2e.xlsx",
      mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      buffer: await readFile(path.join(fixtureRoot, "listino-alfa-medical-2027.xlsx")),
    },
    {
      filename: "storage-e2e.pdf",
      mimeType: "application/pdf",
      buffer: await readFile(path.join(fixtureRoot, "listino-medika-testuale.pdf")),
    },
    {
      filename: "storage-e2e.docx",
