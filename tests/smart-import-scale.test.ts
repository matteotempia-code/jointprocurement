...
import {
  ...
  providerSupportsScannedDocuments,
} from "../src/lib/imports/provider";
...
async function fixtureContext() {
  const user = await prisma.user.findFirstOrThrow({
    where: { name: "Giulia Bianchi" },
    include: { assignments: { take: 1 } },
  });
  const assignment = user.assignments[0];
  const supplier = await prisma.supplier.findFirstOrThrow({
    where: { organizationId: assignment.organizationId, active: true },
  });
...
test("parser e paginazione gestiscono 1.000 righe senza renderizzarle tutte", async () => {
  ...
  const csv =
    ...
    Array.from(
      { length: 1_000 },
      (_, index) =>
        `SKU-${String(index + 1).padStart(4, "0")};Guanto nitrile ${index + 1};2,50;100`,
    ).join("\n");
  ...
  const source = await prisma.sourceDocument.create({
    data: {
      ...
      originalFilename: "fixture-1000.csv",
      mimeType: "text/csv",
      fileSize: Buffer.byteLength(csv),
      checksum: `fixture-large-${process.pid}`,
      sourceType: "CSV",
      documentKind: "PRICE_LIST",
      storagePath: "test/fixture-1000.csv",
      status: "PROCESSED",
    },
  });
  try {
    ...
    await prisma.importedRecord.createMany({
      data: Array.from({ length: 1_000 }, (_, index) => ({
        importJobId: job.id,
        recordIndex: index + 1,
        rawSource: `SKU-${String(index + 1).padStart(4, "0")};Prodotto ${index + 1}`,
        rawFields: { supplierSku: `SKU-${index + 1}` },
        interpretedFields: {
          supplierSku: `SKU-${index + 1}`,
          description: `Prodotto ${index + 1}`,
        },
        normalizedFields: {
          description: `Prodotto ${index + 1}`,
          comparable: true,
          normalizedPrice: (index + 1) / 1000,
          normalizedLabel: "",
          validationErrors: [],
          warnings: [],
        },
        sourceLocator: { row: index + 2 },
        searchText: `sku-${String(index + 1).padStart(4, "0")} prodotto ${index + 1}`,
        supplierSkuText: `SKU-${index + 1}`,
        normalizedPriceValue: (index + 1) / 1000,
        exceptionType: index < 38 ? "UNCERTAIN_MATCH" : null,
        status: index < 38 ? "NEEDS_REVIEW" : "READY",
        requiresReview: index < 38,
        matchConfidence: index < 38 ? 0.7 : 0.95,
      })),
    });
    const first = await getImportRecordPage(prisma, {
      jobId: job.id,
      filter: "attention",
      page: 1,
      pageSize: 25,
    });
    const second = await getImportRecordPage(prisma, {
      jobId: job.id,
      filter: "attention",
      page: 2,
      pageSize: 25,
    });
    const searched = await getImportRecordPage(prisma, {
      jobId: job.id,
      filter: "all",
      search: "SKU-0999",
      pageSize: 25,
...
test("bulk review rifiuta selezioni miste, aggiorna contatori e non duplica audit", async () => {
  ...
  const source = await prisma.sourceDocument.create({
    data: {
      ...
      supplierId: supplier.id,
      uploadedByUserId: user.id,
      originalFilename: "fixture-bulk.csv",
      mimeType: "text/csv",
      fileSize: 10,
      checksum: `fixture-bulk-${process.pid}`,
      sourceType: "CSV",
      documentKind: "OFFER",
      storagePath: "test/fixture-bulk.csv",
      status: "PROCESSED",
    },
  });
  try {
    ...
    for (let index = 0; index < 2; index += 1) {
      const record = await prisma.importedRecord.create({
        data: {
          importJobId: job.id,
          recordIndex: index + 1,
          rawSource: `riga ${index + 1}`,
          rawFields: {},
          interpretedFields: {},
          normalizedFields: { comparable: true },
          sourceLocator: { row: index + 1 },
          normalizedPriceValue: 0.025,
          status: "READY",
          requiresReview: true,
        },
      });
      await prisma.productMatchCandidate.create({
        data: {
          ...
          packagingCompatibility: index === 0,
          recommended: true,
        },
      });
      records.push(record);
    ...
    assert.equal(
      await prisma.importedRecord.count({ where: { importJobId: job.id, status: "CONFIRMED" } }),
      0,
      "una selezione mista deve essere atomica",
    );
    const first = await applyBulkReview(prisma, {
      jobId: job.id,
    ...
    const auditAfterFirst = await prisma.auditEvent.count({
      where: {
        OR: [
          { entityType: "IMPORTED_RECORD", entityId: records[0].id },
    ...
    assert.equal(
      auditAfterFirst,
      2,
      "la decisione bulk deve lasciare audit sia sul record sia sul job",
    );
    assert.equal(
      ...
      auditAfterFirst,
      "la ripetizione non deve duplicare audit",
    );
    assert.equal(
      await prisma.importedRecord.count({ where: { importJobId: job.id, status: "CONFIRMED" } }),
      1,
...
test("immagine senza provider resta persistita e non modifica dati canonici", async () => {
  ...
  const jobId = await ingestDocument({
    ...
    filename: `scansione-${process.pid}.png`,
    mimeType: "image/png",
    supplierId: supplier.id,
    documentKind: "PRICE_LIST",
    organizationId: assignment.organizationId,
  ...
  const job = await prisma.importJob.findUniqueOrThrow({
    where: { id: jobId },
    include: { sourceDocument: true, records: true },
  });
  try {
    ...
    const locator = job.sourceDocument.storageObjectKey
      ? path.join("var", "imports", ...job.sourceDocument.storageObjectKey.split("/"))
      : null;
    if (locator) await rm(locator, { force: true });
    await prisma.sourceDocument.delete({ where: { id: job.sourceDocumentId } });
    if (previousStorageProvider === undefined) delete process.env.DOCUMENT_STORAGE_PROVIDER;
    else process.env.DOCUMENT_STORAGE_PROVIDER = previousStorageProvider;
  }
});
