...
test("demo seed requires both an explicit DEV marker and deliberate opt-in", () => {
  ...
  assert.throws(
    () => assertDemoSeedSafety({ SORGENCE_ENVIRONMENT: "development" }),
    /Seed demo bloccato/,
  );
  assert.throws(() => assertDemoSeedSafety({ ALLOW_DEMO_SEED: "true" }), /Seed demo bloccato/);
  assert.doesNotThrow(() =>
    assertDemoSeedSafety({ SORGENCE_ENVIRONMENT: "development", ALLOW_DEMO_SEED: "true" }),
  );
});
  ...
  assert.throws(
    () =>
      assertDemoSeedSafety({
        NODE_ENV: "production",
        SORGENCE_ENVIRONMENT: "development",
