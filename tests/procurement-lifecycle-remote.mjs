...
assert.equal(
  process.env.VERCEL_TARGET,
  "develop",
  "remote lifecycle certification is develop-only",
);
assert.equal(
  process.env.DEMO_MODE,
  "true",
  "remote lifecycle certification requires DEMO_MODE=true",
);
assert.match(
...
page.on("requestfailed", (request) => {
  if (request.method() === "POST" && new URL(request.url()).origin === new URL(base).origin)
    lastActionFailure =
      request
        .failure()
        ?.errorText?.replace(/[^A-Za-z0-9_. -]/g, "")
        .slice(0, 80) || "unknown";
});
...
async function assertNoOverflow() {
  assert.equal(
    ...
    false,
    `horizontal overflow at ${new URL(page.url()).pathname}`,
  );
}
...
async function certifyRoutes(name, allowed, denied) {
  ...
  for (const route of denied) {
    ...
    assert.equal(
      ...
      route,
      `denied direct route remains bounded at ${route}`,
    );
  }
}
...
async function clearCart() {
  ...
  if (!originalCartLines.length && cart)
    originalCartLines = cart.lines.map(({ canonicalProductId, supplierOfferId, quantity }) => ({
      canonicalProductId,
      supplierOfferId,
      quantity,
    }));
...
async function ensureLifecycleFixtures(organizationId, userId) {
  const category = await db.category.upsert({
    ...
    create: {
      ...
      organizationId,
      code: FIXTURE.prefix,
      name: "Certification lifecycle category",
    },
    update: { organizationId, name: "Certification lifecycle category" },
  });
  await db.technicalEvidenceRequirement.upsert({
    where: {
      organizationId_categoryId_requirementType_label: {
        organizationId,
        categoryId: category.id,
        requirementType: "DOCUMENT",
        label: "CERT M11 technical sheet",
      },
    },
    create: {
      organizationId,
  ...
  const supplier = await db.supplier.upsert({
    ...
    create: {
      ...
      organizationId,
      name: "CERT M11 Lifecycle Supplier",
      vatNumber: "ITCERTM110000001",
      active: true,
    },
  ...
  await db.priceList.upsert({
    ...
    create: {
      id: FIXTURE.priceListId,
      organizationId,
      name: "CERT M11 Lifecycle Price List",
  ...
  const products = [
    {
      id: FIXTURE.normalProductId,
      name: "CERT M11 Normal Product",
      sku: "CERT-M11-NORMAL",
      ean: "9900000000011",
      offerId: FIXTURE.normalOfferId,
      price: 12,
    },
    {
      id: FIXTURE.limitedProductId,
      name: "CERT M11 Limited Product",
      sku: "CERT-M11-LIMITED",
      ean: "9900000000028",
      offerId: FIXTURE.limitedOfferId,
      price: 15,
    },
  ];
  for (const fixture of products) {
    await db.canonicalProduct.upsert({
      ...
      create: {
        ...
        organizationId,
        name: fixture.name,
        description: `${FIXTURE.prefix} deterministic fixture`,
        manufacturer: "Certification Industries",
        manufacturerSku: fixture.sku,
        ean: fixture.ean,
        uom: "EA",
        purchaseUom: "PACK",
        unitsPerPackage: 1,
    ...
    await db.supplierOffer.upsert({
      ...
      create: {
        ...
        organizationId,
        supplierId: supplier.id,
        canonicalProductId: fixture.id,
        priceListId: FIXTURE.priceListId,
        supplierSku: fixture.sku,
        unitPrice: fixture.price,
        normalizedUnitPrice: fixture.price,
        moq: 1,
        taxRate: 22,
        preferred: true,
        active: true,
    ...
    const sourceId = `${fixture.id}_source`,
      documentId = `${fixture.id}_document`,
      versionId = `${fixture.id}_version`;
    await db.sourceDocument.upsert({
      ...
      create: {
        ...
        originalFilename: `${fixture.sku}.txt`,
        mimeType: "text/plain",
        fileSize: 128,
        checksum: `${fixture.id}_sha256`,
        sourceType: "TXT",
        documentKind: "OTHER",
        storagePath: `certification/${fixture.sku}.txt`,
        storageProvider: "fixture",
        status: "PROCESSED",
        metadata: { certificationFixture: FIXTURE.prefix },
      },
      update: {
        organizationId,
        uploadedByUserId: userId,
    ...
    await db.technicalDocument.upsert({
      ...
      create: {
        id: documentId,
        organizationId,
        familyKey: fixture.sku.toLowerCase(),
        documentType: "TECHNICAL_SHEET",
        title: `${fixture.name} technical sheet`,
        manufacturer: "Certification Industries",
        status: "READY",
      },
    ...
    await db.technicalDocumentVersion.upsert({
      ...
      create: {
        ...
        checksum: `${fixture.id}_sha256`,
        extractedMetadata: { certificationFixture: FIXTURE.prefix, manufacturerSku: fixture.sku },
        interpretationProvider: "CERTIFICATION_FIXTURE",
        confidence: 1,
        status: "READY",
      },
      update: { validUntil: new Date("2100-01-01T00:00:00Z"), status: "READY" },
    });
    await db.technicalDocument.update({
      where: { id: documentId },
    ...
    await db.technicalDocumentProductAssociation.upsert({
      ...
      create: {
        technicalDocumentId: documentId,
        canonicalProductId: fixture.id,
        associationType: "MANUFACTURER_SKU_EXACT",
        confidence: 1,
        status: "MANUALLY_CONFIRMED",
        evidence: [`SKU ${fixture.sku}`, FIXTURE.prefix],
        explanation: "Dedicated certification evidence",
        decisionSource: "CERTIFICATION_FIXTURE",
    ...
    await db.productTechnicalState.upsert({
      where: {
        organizationId_canonicalProductId: { organizationId, canonicalProductId: fixture.id },
      },
      create: {
        organizationId,
        canonicalProductId: fixture.id,
        ...
        evidenceFingerprint: `${FIXTURE.prefix}:${fixture.id}:CERT-1`,
      },
      update: {
        status: "COMPLETE",
        completenessPercent: 100,
  ...
  await db.procurementLimit.upsert({
    ...
    create: {
      ...
      organizationId,
      facilityId,
      canonicalProductId: FIXTURE.limitedProductId,
      limitType: "QUANTITY",
...
async function addSelectedProduct(quantity) {
  ...
  await waitForDb(
    () =>
      db.cartLine.findFirst({
        where: {
          cart: { userId: lucia.id, facilityId },
          supplierOfferId: selectedProduct.offer.id,
        },
...
async function submitRequest(quantity, scenario) {
  ...
  const cartLine = await db.cartLine.findFirst({
    where: { cart: { userId: lucia.id, facilityId } },
    select: { quantity: true },
  });
  ...
  const request = await db.purchaseRequisition.findUnique({
    where: { id },
    include: { lines: true, approvals: true, purchaseOrders: { include: { lines: true } } },
  });
...
async function decide(approvalId, decision, note = "") {
  ...
  const approval = await db.approvalRequest.findUniqueOrThrow({
    where: { id: approvalId },
    include: { requisition: { include: { purchaseOrders: true } } },
  });
...
async function receive(poId, mode) {
  ...
  return db.purchaseOrder.findUniqueOrThrow({
    where: { id: poId },
    include: {
      receipts: { include: { lines: { include: { qualityIssues: true } } } },
...
async function cleanup() {
  ...
  const requests = await db.purchaseRequisition.findMany({
    where: {
      OR: [{ id: { in: createdRequisitionIds } }, { justification: { startsWith: marker } }],
    },
    include: {
      purchaseOrders: { include: { lines: true, receipts: { include: { lines: true } } } },
    },
  });
  const requestIds = requests.map(({ id }) => id);
  const poIds = requests.flatMap((request) => request.purchaseOrders.map(({ id }) => id));
  const poLineIds = requests.flatMap((request) =>
    request.purchaseOrders.flatMap((order) => order.lines.map(({ id }) => id)),
  );
  const receiptIds = requests.flatMap((request) =>
    request.purchaseOrders.flatMap((order) => order.receipts.map(({ id }) => id)),
  );
  const receiptLineIds = requests.flatMap((request) =>
    request.purchaseOrders.flatMap((order) =>
      order.receipts.flatMap((receipt) => receipt.lines.map(({ id }) => id)),
    ),
  );
  await db.$transaction(async (tx) => {
    if (poLineIds.length || receiptLineIds.length)
      ...
      await tx.auditEvent.deleteMany({
        where: {
          OR: [
            ...
            { entityId: { in: poIds } },
            { entityId: { in: poLineIds } },
            { entityId: { in: receiptIds } },
            { entityId: { in: receiptLineIds } },
          ],
        },
      });
    ...
    if (favoriteBefore !== null) {
      await tx.favorite.deleteMany({
        where: { userId: lucia.id, facilityId, canonicalProductId: favoriteProductId },
      });
      if (favoriteBefore > 0)
        await tx.favorite.create({
          data: { userId: lucia.id, facilityId, canonicalProductId: favoriteProductId },
        });
    }
    ...
    if (originalCartLines.length)
      await tx.cartLine.createMany({
        data: originalCartLines.map((line) => ({ cartId: cart.id, ...line })),
      });
  });
}
...
try {
  ...
  lucia = await db.user.findFirstOrThrow({
  ...
  const [normalTechnicalState, limitedTechnicalState, normalLimitCount] = await Promise.all([
    db.productTechnicalState.findUnique({
      where: {
        organizationId_canonicalProductId: {
          organizationId,
          canonicalProductId: FIXTURE.normalProductId,
        },
      },
    }),
    db.productTechnicalState.findUnique({
  ...
  const technicallyPurchasable = {
    none: { organizationId: lucia.assignments[0].organizationId, status: { not: "COMPLETE" } },
  };
  const product = await db.canonicalProduct.findFirstOrThrow({
    where: {
      ...
      offers: { some: { id: FIXTURE.normalOfferId, active: true, preferred: true } },
    },
    include: {
      offers: { where: { active: true, preferred: true }, orderBy: { unitPrice: "asc" }, take: 1 },
    },
    orderBy: { name: "asc" },
  });
  ...
  await certifyRoutes(
    ...
    ["/imports", "/approvals", "/organization"],
  );
  await certifyRoutes(
    "Andrea Riva",
    [
  ...
  await certifyRoutes(
    ...
    ["/cart", "/approvals", "/orders"],
  );
  await certifyRoutes(
    "Elena Conti",
    ["/"],
    ["/cart", "/approvals", "/imports", "/organization", "/control-tower"],
  );
  await certifyRoutes(
    "Davide Romano",
    ["/", "/control-tower"],
    ["/cart", "/approvals", "/imports", "/organization"],
  );
  ...
  favoriteBefore = await db.favorite.count({
    where: { userId: lucia.id, facilityId, canonicalProductId: product.id },
  });
  ...
  assert.equal(
    await favoriteAction.count(),
    1,
    "Product 360 exposes one favorite action for the active scope",
  );
  ...
  await page
    .locator(".phase2-create-popover form")
    .getByRole("button", { name: "Crea lista" })
    .click();
  ...
  await waitForDb(
    () =>
      db.shoppingListItem.count({
        where: { shoppingListId: listId, canonicalProductId: product.id },
      }),
    (count) => count === 1,
    "shopping-list item",
  );
  ...
  const unitGross =
    Number(selectedProduct.offer.unitPrice) * (1 + Number(selectedProduct.offer.taxRate) / 100);
  ...
  assert.equal(
    approvalRequest.approvals[0].approverUserId,
    (await db.user.findFirstOrThrow({ where: { name: "Andrea Riva" } })).id,
  );
  ...
  await decide(
    clarificationRequest.approvals[0].id,
    "CLARIFICATION_REQUESTED",
    `${marker} specificare fabbisogno`,
  );
  await switchTo("Lucia Ferri");
  ...
  const clarified = await db.purchaseRequisition.findUniqueOrThrow({
    where: { id: clarificationRequest.id },
    include: { approvals: { orderBy: { requestedAt: "desc" } } },
  });
  ...
  assert.ok(
    withIssue.receipts.flatMap((receipt) => receipt.lines.flatMap((line) => line.qualityIssues))
      .length,
  );
  ...
  const rejected = await decide(
    rejectionRequest.approvals[0].id,
    "REJECTED",
    `${marker} motivazione insufficiente`,
  );
  ...
  assert.ok(
    boundaryRequest.approvals.length,
    "limit exception creates a governed approval request",
  );
  ...
  assert.match(
    ...
    /riconciliazione fatture.*non sono ancora operativi/is,
  );
  ...
  await page
    .screenshot({ path: path.join(directory, "procurement-lifecycle-failure.png"), fullPage: true })
    .catch(() => undefined);
  const safe = (error instanceof Error ? error.message : String(error))
    .replace(/https?:\/\/\S+/g, "[url]")
    .replace(/\s+/g, " ")
    .slice(0, 600);
  const correlated = await db.purchaseRequisition
    .findFirst({
      where: { justification: { startsWith: marker } },
      orderBy: { createdAt: "desc" },
      select: { _count: { select: { approvals: true, purchaseOrders: true } } },
    })
    .catch(() => null);
  const favoriteAfterFailure =
    favoriteBefore === null
      ? null
      : await db.favorite
          .count({ where: { userId: lucia.id, facilityId, canonicalProductId: favoriteProductId } })
          .catch(() => null);
  const mutationPersisted =
    favoriteBefore === null || favoriteAfterFailure === null
      ? "unknown"
      : favoriteAfterFailure !== favoriteBefore
        ? "persisted"
        : "not-persisted";
  ...
  const diagnostic = JSON.stringify({
    ...
    requestFound: Boolean(correlated),
    approvalCount: correlated?._count.approvals ?? 0,
    purchaseOrderCount: correlated?._count.purchaseOrders ?? 0,
    actionStatus: lastActionStatus,
    mutationPersisted,
  });
  if (process.env.GITHUB_ACTIONS === "true")
    console.error(
      `::error title=Remote lifecycle ${checkpoint}-${lastActionStatus}-${mutationPersisted}-${lastActionFailure}-${browserFailure}::${safe} | ${diagnostic}`,
    );
  throw error;
} finally {
  await cleanup().catch((error) =>
    console.error(
      "REMOTE_LIFECYCLE_CLEANUP_FAILED",
