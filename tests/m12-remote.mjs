...
page.on("console", (message) => {
  if (message.type() === "error" && !/tree hydrated/i.test(message.text()))
...
async function waitFor(read, predicate, label, timeout = 300_000) {
  ...
  assert.fail(`${label} did not converge`);
}
const text = (product, suffix, overrides = "") =>
  Buffer.from(
...
try {
  ...
  const excludedProducts = limits.flatMap((limit) =>
    limit.canonicalProductId ? [limit.canonicalProductId] : [],
  );
  const excludedCategories = limits.flatMap((limit) =>
    limit.categoryId ? [limit.categoryId] : [],
  );
  const product = await db.canonicalProduct.findFirstOrThrow({
    where: {
      active: true,
      id: { notIn: excludedProducts },
      categoryId: { notIn: excludedCategories },
      manufacturerSku: { not: null },
      offers: {
        some: { active: true, normalizedUnitPrice: { not: null }, supplier: { active: true } },
      },
    },
  ...
  const files = Array.from({ length: 100 }, (_, index) => ({
    name: `${marker}-technical-${String(index).padStart(3, "0")}.txt`,
    mimeType: "text/plain",
    buffer: text(product, `Documento ${index}`),
  }));
  ...
  assert.equal(
    await aiToggle.isChecked(),
    false,
    "100-document batch must have Procurement AI disabled at submit",
  );
  const started = Date.now();
  ...
  const batch = await waitFor(
    () =>
      db.technicalDocumentBatch.findFirst({
        where: {
          createdBy: { name: "Giulia Bianchi" },
          createdAt: { gte: new Date(started - 5_000) },
        },
    ...
    (value) =>
      ...
      value.completedFiles + value.failedFiles === 100,
  ...
  assert.ok(
    batch.items.every((item) =>
      ["COMPLETED", "REVIEW_REQUIRED", "NEEDS_OCR"].includes(item.status),
    ),
  );
  assert.equal(
    new Set(batch.items.map((item) => item.sourceDocumentId)).size,
    100,
    "each file has one source locator",
  );
  assert.ok(
    batch.items.every((item) => item.attempts === 1),
  ...
  await page.locator('input[name="files"]').setInputFiles([
    {
      name: `${marker}-image-only.png`,
      mimeType: "image/png",
      buffer: Buffer.concat([Buffer.from([137, 80, 78, 71, 13, 10, 26, 10]), Buffer.from(marker)]),
    },
    {
      name: `${marker}-malformed.pdf`,
      mimeType: "application/pdf",
      buffer: Buffer.from(`this is not a PDF ${marker}`),
    },
  ...
  const negativeBatch = await waitFor(
    () =>
      db.technicalDocumentBatch.findFirst({
        where: { createdAt: { gte: negativeStarted } },
        orderBy: { createdAt: "desc" },
        include: { items: true },
      }),
    (value) =>
      value &&
      ["PARTIAL", "FAILED"].includes(value.status) &&
      value.items.every((item) =>
        ["FAILED", "NEEDS_OCR", "COMPLETED", "REVIEW_REQUIRED"].includes(item.status),
      ),
  ...
  assert.equal(
    negativeBatch.items.filter((item) => item.status === "NEEDS_OCR").length,
    1,
    "image-only content requires OCR",
  );
  const failedItem = negativeBatch.items.find((item) => item.status === "FAILED");
  assert.ok(failedItem);
  assert.equal(
    failedItem.attempts,
    failedItem.maxAttempts,
    "transient retry budget is exhausted before surfacing failure",
  );
  await open("/technical-documents");
  ...
  const associations = await db.technicalDocumentProductAssociation.count({
    where: { canonicalProductId: product.id, technicalDocument: { batchId: batch.id } },
  });
  assert.equal(associations, 100);
  const state = await db.productTechnicalState.findUnique({
    where: {
  ...
  for (let attempt = 1; attempt <= 3 && !aiCall; attempt += 1) {
    ...
    await page.locator('input[name="files"]').setInputFiles({
      name: `${marker}-openai-${attempt}.txt`,
      mimeType: "text/plain",
      buffer: text(
        product,
        `OpenAI classification ${attempt}`,
    ...
    await waitFor(
      () =>
        db.technicalDocumentBatch.findFirst({
          where: { createdAt: { gte: attemptStarted }, aiEnabled: true },
          orderBy: { createdAt: "desc" },
        }),
      (value) => value && ["COMPLETED", "PARTIAL"].includes(value.status),
      `AI batch ${attempt}`,
    );
    const call = await waitFor(
      () =>
        db.procurementAICall.findFirst({
          where: { operation: "TECHNICAL_DOCUMENT", createdAt: { gte: attemptStarted } },
          orderBy: { createdAt: "desc" },
        }),
      Boolean,
      `OpenAI invocation ${attempt}`,
      180_000,
    );
  ...
  assert.ok(
    aiCall,
    `OpenAI did not return a validated result after 3 invocations since ${aiStarted.toISOString()}`,
  );
  ...
  await open("/technical-documents");
  assert.equal(
    errors.filter((message) => !/server components/i.test(message)).length,
    0,
    errors.join(" | "),
  );
  console.log(
    JSON.stringify({
      ...
      negativeBatch: { partialFailure: true, needsOcr: true, duplicate: true, retry: true },
      associationCount: associations,
      productTechnicalStatus: state.status,
      openAI: { provider: aiCall.provider, model: aiCall.model, resultState: aiCall.resultState },
      authorization: true,
    }),
  );
} catch (error) {
  ...
  const diagnosticBatch = diagnosticBatchId
    ? await db.technicalDocumentBatch
        .findUnique({
          where: { id: diagnosticBatchId },
          include: { items: { select: { status: true, attempts: true, lastError: true } } },
        })
        .catch(() => null)
    : await db.technicalDocumentBatch
        .findFirst({
          where: { createdBy: { name: "Giulia Bianchi" } },
          orderBy: { createdAt: "desc" },
          include: { items: { select: { status: true, attempts: true, lastError: true } } },
        })
        .catch(() => null);
  if (diagnosticBatch) {
    ...
    await writeFile(
      path.join(directory, `m12-${checkpoint}-batch.json`),
      JSON.stringify(
        {
          batch: {
            id: diagnosticBatch.id,
            status: diagnosticBatch.status,
            aiEnabled: diagnosticBatch.aiEnabled,
            totalFiles: diagnosticBatch.totalFiles,
            completedFiles: diagnosticBatch.completedFiles,
            failedFiles: diagnosticBatch.failedFiles,
          },
          itemGroups: [...groups].map(([key, count]) => ({ ...JSON.parse(key), count })),
        },
        null,
        2,
      ),
  ...
  await page
    .screenshot({ path: path.join(directory, `m12-${checkpoint}.png`), fullPage: true })
    .catch(() => undefined);
  ...
  if (process.env.GITHUB_ACTIONS === "true")
    console.error(`::error title=M12 ${checkpoint}::${safe}`);
  throw error;
} finally {
  await context.close();
  await browser.close();
