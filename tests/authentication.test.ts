...
const demoEnvironment = {
  DEMO_MODE: "true",
  DEMO_SESSION_SECRET: "a-test-secret-that-is-long-and-random",
};
...
test("demo identity is refused unless DEMO_MODE is exactly true", () => {
  ...
  assert.throws(
    () => sealDemoUserId("user-1", { DEMO_MODE: "false", DEMO_SESSION_SECRET: "secret" }),
    /disabled/,
  );
  assert.throws(
    () => openDemoUserId("token", { DEMO_MODE: "false", DEMO_SESSION_SECRET: "secret" }),
    /disabled/,
  );
});
