...
test("demo facility and area assignments resolve to their exact seeded scope", async () => {
  const assignments = await prisma.userAssignment.findMany({
    where: { user: { email: { in: ["lucia.ferri@demo.local", "andrea.riva@demo.local"] } } },
    include: { user: true },
  });
  ...
  for (const expected of ["Casa Serena", "RSA Aurora", "Residenza San Michele"])
    assert.ok(areaFacilityNames.includes(expected));
});
