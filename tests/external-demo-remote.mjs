...
try {
  ...
  await switchTo("Lucia Ferri");for(const invalid of ["0","-1","2"]){await open(`/orders/${edgePo.id}/receive`);const receiptInput=page.locator('input[name^="received-"]').first();await receiptInput.fill(invalid);await page.getByRole("button",{name:"Conferma tutto come ordinato"}).click();await page.waitForURL(invalid==="0"?/error=empty-receipt/:/error=invalid-quantity/);assert.equal(await db.receipt.count({where:{purchaseOrderId:edgePo.id}}),0,`receipt ${invalid} does not mutate`);await page.locator('.warning[role="alert"]').waitFor();}
