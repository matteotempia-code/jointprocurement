...
export default async function OrderDetailLayout({
  ...
  const order = await prisma.purchaseOrder.findFirst({
    where: { id: (await params).id, facilityId: scope.id },
  ...
  return (
    <>
      <div className="context-action-bar">
        <div>
          <strong>Acquisto ricorrente?</strong>
          <span>Ritrova facilmente prodotti e quantità di {order.poNumber}.</span>
        </div>
        <div className="context-actions">
          <form action={addOrderProductsToFavorites}>
            <input type="hidden" name="poId" value={order.id} />
            <button>Aggiungi ai preferiti</button>
          </form>
          <form action={createListFromOrder}>
            <input type="hidden" name="poId" value={order.id} />
            <button className="secondary-cta">Crea lista da questo ordine</button>
          </form>
        </div>
      </div>
      {children}
