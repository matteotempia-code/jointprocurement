...
export async function confirmPriceListCondition(formData: FormData) {
  ...
  await prisma.$transaction([
    ...
    prisma.auditEvent.create({ data: { organizationId: context.organization.id, actorUserId: context.user.id, entityType: "PRICE_LIST_CONDITION", entityId: condition.id, action: "COMMERCIAL_CONDITION_CONFIRMED", metadata: { priceListId: condition.priceListId, conditionType: condition.conditionType } } }),
  ]);
