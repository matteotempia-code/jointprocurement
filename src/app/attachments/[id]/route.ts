...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const attachment = await prisma.operationalAttachment.findFirst({
    where: {
      ...
      ...(context.roleCode === "PROCUREMENT_MANAGER" || context.roleCode === "PROCUREMENT_ADMIN"
        ? {}
  ...
  try {
    ...
    const storage = getDocumentStorage(provider),
      locator = {
        provider,
    ...
    return new NextResponse(new Uint8Array(data), {
      headers: {
        ...
        "Content-Disposition": `inline; filename="${attachment.originalFilename.replace(/["\r\n]/g, "-")}"`,
        "X-Content-Type-Options": "nosniff",
        "Cache-Control": "private, no-store",
      },
    });
  } catch {
    return new NextResponse("Il file non è disponibile nello storage configurato.", {
      status: 404,
    });
  }
}
