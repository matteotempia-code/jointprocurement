  ...
  return (
    <main className="phase2-page phase2-issues">
      <PageHeader
        ...
        description={`${total} casi nel filtro · evidenze e risoluzioni tracciate`}
      />
      ...
      {issues.length ? (
        <section className="phase2-issue-list">
          {issues.map((issue) => {
            const order = issue.purchaseOrderLine.purchaseOrder,
              closed = ["RESOLVED", "CLOSED"].includes(issue.status);
            return (
              <article key={issue.id}>
                <header>
                  <div>
                    <span>
                      {statusLabel(issue.issueType)} · {order.facility.name}
                    </span>
                    <h2>{issue.purchaseOrderLine.canonicalProduct.name}</h2>
                    <p>{issue.description}</p>
                  </div>
                  <StatusChip
                    variant={closed ? "ok" : issue.severity === "CRITICAL" ? "danger" : "warn"}
                  >
                    {statusLabel(issue.severity)} · {statusLabel(issue.status)}
                  </StatusChip>
                </header>
                <div className="phase2-issue-meta">
                  <Link href={`/orders/${order.id}`}>{order.poNumber}</Link>
                  <span>{order.supplier.name}</span>
                  <span>Aperta {formatDate(issue.openedAt)}</span>
                  <span>{Number(issue.affectedQuantity)} unità</span>
                </div>
                ...
                {context.roleCode === "PROCUREMENT_MANAGER" && !closed && (
                  <details className="phase2-resolution">
                    <summary>Prendi in carico o risolvi</summary>
                    <form action={resolveQualityIssue}>
                      ...
                      />
                      <button className="primary-cta">Salva decisione</button>
                    </form>
                  </details>
                )}
              </article>
