...
export default async function FacilityDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main><PageHeader eyebrow={facility.area.name} title={facility.name} description={facility.area.legalEntity.name} action={<ScopeBadge type="AREA" label={scope.label} />} /><section className="section-heading"><div><p className="eyebrow">Organization structure</p><h2>Cost centers</h2></div></section><DataTable label="Cost centers"><thead><tr><th>Code</th><th>Name</th><th>Facility</th></tr></thead><tbody>{facility.costCenters.map((center) => <tr key={center.id}><td className="mono">{center.code}</td><td><strong>{center.name}</strong></td><td>{facility.name}</td></tr>)}</tbody></DataTable></main>;
}
