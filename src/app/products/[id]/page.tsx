...
export default async function Product360({ params }: { params: Promise<{ id: string }> }) {
  ...
  const product = await prisma.canonicalProduct.findUnique({ where: { id: (await params).id }, include: { category: true, offers: { where: { active: true }, include: { supplier: true, priceList: true, priceHistory: { orderBy: { effectiveAt: "asc" } } } } } });
  if (!product) notFound();
  ...
  const preferredPrice = preferred ? normalizeOfferPrice(product, preferred) : null;
  ...
  const history = preferred?.priceHistory ?? [];
  ...
  return <main>
    ...
    <section className="product-hero refined">
      ...
      <aside className="buy-box"><p className="eyebrow">Offerta convenzionata</p><h3>{preferred?.supplier.name ?? "Non disponibile"}</h3><strong>{preferredPrice ? formatMoney(preferredPrice.purchasePrice) : "—"}</strong><span>per {preferredPrice?.purchaseLabel}</span><small>{preferredPrice?.contentLabel}</small><b>{preferredPrice?.normalizedPrice != null ? `${formatCurrency(preferredPrice.normalizedPrice, 4)} / ${preferredPrice.consumptionLabel}` : preferredPrice?.normalizedLabel}</b><small>Consegna stimata: {preferred?.leadTimeDays ?? "—"} giorni</small>
        {context.roleCode === "RSA_DIRECTOR" && preferred && <div className="buy-actions">
          <form action={addToCart}><input type="hidden" name="offerId" value={preferred.id} /><label>Quantità<input name="quantity" type="number" min={Number(preferred.moq)} defaultValue={Number(preferred.moq)} /></label><button className="primary-cta">Aggiungi al carrello</button></form>
    ...
    <section className="commercial-summary"><div><span>Disponibilità</span><strong>{statusLabel(preferred?.availabilityStatus ?? "UNAVAILABLE")}</strong><small>Consegna in {preferred?.leadTimeDays ?? "—"} giorni</small></div><div><span>Migliore disponibile</span><strong>{bestPrice?.normalizedPrice != null ? formatCurrency(bestPrice.normalizedPrice, 4) : "—"}</strong><small>{comparison.lowest?.supplier.name}</small></div><div><span>Posizione convenzionato</span><strong>{comparison.preferredDelta === 0 ? "Miglior prezzo" : `+${comparison.preferredDelta.toFixed(1)}%`}</strong><small>{comparison.preferredDelta === 0 ? "Nessun differenziale" : "Da valutare"}</small></div></section>
