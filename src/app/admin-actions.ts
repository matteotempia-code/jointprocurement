...
async function audit(organizationId: string, actorUserId: string, entityType: string, entityId: string, action: string, metadata: Record<string, unknown> = {}) {
  await prisma.auditEvent.create({ data: { organizationId, actorUserId, entityType, entityId, action, metadata: metadata as Prisma.InputJsonValue } });
}
...
export async function updateOrganization(formData: FormData) {
  ...
  await audit(context.organization.id, context.user.id, "ORGANIZATION", context.organization.id, "UPDATED", { name });
...
export async function manageLegalEntity(formData: FormData) {
  ...
  if (intent === "create") {
    ...
    await audit(context.organization.id, context.user.id, "LEGAL_ENTITY", entity.id, "CREATED", { name });
  } else {
    ...
    if (intent === "update") {
      ...
      await audit(context.organization.id, context.user.id, "LEGAL_ENTITY", id, "UPDATED", { name });
    } else if (intent === "delete") {
      if (entity.areas.length) target("/organization", "entita-in-uso");
      await audit(context.organization.id, context.user.id, "LEGAL_ENTITY", id, "DELETED", { name: entity.name });
...
export async function manageUser(formData: FormData) {
  ...
  if (intent === "create") {
    ...
    await audit(context.organization.id, context.user.id, "USER", user.id, "CREATED", { email, roleId, scopeType, scopeId });
  } else {
    ...
    if (intent === "update") {
      ...
      await audit(context.organization.id, context.user.id, "USER", assignment.userId, "UPDATED", { roleId, scopeType, scopeId });
      ...
      await audit(context.organization.id, context.user.id, "USER", assignment.userId, "DEACTIVATED", { assignmentId });
    } else if (intent === "delete") {
...
export async function manageDelegation(formData: FormData) {
  ...
  if (intent === "create") {
    ...
    await audit(context.organization.id, context.user.id, "APPROVAL_DELEGATION", row.id, "CREATED");
  } else {
    ...
    if (intent === "update") {
      ...
      await audit(context.organization.id, context.user.id, "APPROVAL_DELEGATION", row.id, "UPDATED");
    } else {
      await prisma.approvalDelegation.update({ where: { id }, data: { active: false } });
      await audit(context.organization.id, context.user.id, "APPROVAL_DELEGATION", row.id, "DEACTIVATED");
    }
  }
...
export async function manageSupplier(formData: FormData) {
  ...
  if (intent === "create") {
    ...
    if (vatNumber && await prisma.supplier.findUnique({ where: { organizationId_vatNumber: { organizationId: context.organization.id, vatNumber } } })) target("/suppliers", "duplicato");
    const row = await prisma.supplier.create({ data: { organizationId: context.organization.id, name, vatNumber, contactEmail } }); await audit(context.organization.id, context.user.id, "SUPPLIER", row.id, "CREATED");
  } else {
    const row = await prisma.supplier.findFirstOrThrow({ where: { id, organizationId: context.organization.id } });
    if (intent === "update") { await prisma.supplier.update({ where: { id }, data: { name, vatNumber, contactEmail } }); await audit(context.organization.id, context.user.id, "SUPPLIER", id, "UPDATED"); }
    else { await prisma.supplier.update({ where: { id }, data: { active: false, offers: { updateMany: { where: { active: true }, data: { active: false } } } } }); await audit(context.organization.id, context.user.id, "SUPPLIER", row.id, "DEACTIVATED"); }
  }
...
export async function manageProduct(formData: FormData) {
  ...
  if (intent === "create") {
    if (name.length < 3 || !await prisma.category.findFirst({ where: { id: categoryId, organizationId: context.organization.id } })) target("/products", "dati-non-validi");
    const duplicate = await prisma.canonicalProduct.findFirst({ where: { organizationId: context.organization.id, name: { equals: name, mode: "insensitive" }, categoryId } }); if (duplicate) target("/products", "duplicato");
    const row = await prisma.canonicalProduct.create({ data: { organizationId: context.organization.id, name, categoryId, uom, purchaseUom: uom, unitsPerPackage: 1, consumptionUom: "PIECE", active: true } }); await audit(context.organization.id, context.user.id, "CANONICAL_PRODUCT", row.id, "CREATED");
  } else {
    const row = await prisma.canonicalProduct.findFirstOrThrow({ where: { id, organizationId: context.organization.id } });
    if (intent === "update") { await prisma.canonicalProduct.update({ where: { id }, data: { name, categoryId, uom, purchaseUom: uom } }); await audit(context.organization.id, context.user.id, "CANONICAL_PRODUCT", id, "UPDATED"); }
    else { await prisma.canonicalProduct.update({ where: { id }, data: { active: false, offers: { updateMany: { where: { active: true }, data: { active: false } } } } }); await audit(context.organization.id, context.user.id, "CANONICAL_PRODUCT", row.id, "DEACTIVATED"); }
  }
...
export async function manageCategory(formData: FormData) {
  ...
  if (intent === "create") {
    ...
    if (await prisma.category.findUnique({ where: { organizationId_code: { organizationId: context.organization.id, code } } })) target("/categorie", "duplicato");
    const row = await prisma.category.create({ data: { organizationId: context.organization.id, name, code } }); await audit(context.organization.id, context.user.id, "CATEGORY", row.id, "CREATED");
  } else {
    const row = await prisma.category.findFirstOrThrow({ where: { id, organizationId: context.organization.id }, include: { _count: { select: { products: true, budgets: true, procurementLimits: true } } } });
    if (intent === "update") { await prisma.category.update({ where: { id }, data: { name, code } }); await audit(context.organization.id, context.user.id, "CATEGORY", id, "UPDATED"); }
    else { if (Object.values(row._count).some(Boolean)) target("/categorie", "categoria-in-uso"); await audit(context.organization.id, context.user.id, "CATEGORY", id, "DELETED", { name: row.name }); await prisma.category.delete({ where: { id } }); }
  }
