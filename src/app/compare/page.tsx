...
export default async function ConfrontoPrezzi({ searchParams }: { searchParams: Promise<{ q?: string; pagina?: string }> }) {
  ...
  const comparisons = products.filter((item) => item.offers.length > 1).map((product) => ({ product, result: compareOffers(product.offers, context.organization.vatDeductibilityPercent) })), pages = Math.max(1, Math.ceil(total / PAGE_SIZE));
  return <main className="phase2-page phase2-compare"><PageHeader eyebrow="Intelligence prezzi" title="Confronto offerte" description="Le differenze normalizzate sono dominanti; gli attributi identici restano secondari." />
    ...
    <div className="phase2-comparison-list">{comparisons.map(({ product, result }) => <article key={product.id}><header><div><span>{product.category.name}</span><Link href={`/products/${product.id}`}>{product.name}</Link></div><StatusChip variant={result.spread > 20 ? "warn" : "neutral"}>{result.spread.toLocaleString("it-IT", { maximumFractionDigits: 1 })}% differenza</StatusChip></header><div>{result.sorted.slice(0, 4).map((offer, index) => <div className={index === 0 ? "is-best" : ""} key={offer.id}><span>{offer.supplier.name}{offer.preferred ? " Â· convenzionato" : ""}</span><strong><Num value={getComparablePrice(offer, context.organization.vatDeductibilityPercent)} kind="currency" digits={4} /> / unitÃ  normalizzata</strong><StatusChip variant={index === 0 ? "ok" : "neutral"}>{index === 0 ? "Migliore" : `+${((getComparablePrice(offer, context.organization.vatDeductibilityPercent) / getComparablePrice(result.lowest!, context.organization.vatDeductibilityPercent) - 1) * 100).toLocaleString("it-IT", { maximumFractionDigits: 1 })}%`}</StatusChip></div>)}</div></article>)}</div>
