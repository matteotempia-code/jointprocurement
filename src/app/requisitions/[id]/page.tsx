...
export default async function RequisitionDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ created?: string; clarification?: string }> }) {
  ...
  return <main className="phase2-page phase2-cockpit">
    ...
    <section className="policy-panel"><p className="eyebrow">Esito della policy</p><h2>{statusLabel(request.policyDecision)}</h2><p>{policyExplanationLabel(request.policyExplanation)}</p>{budgetWarning && <p className="warning">Attenzione budget: la richiesta resta consentita, ma porta l’utilizzo del periodo almeno all’80%.</p>}{request.justification && <blockquote>{request.justification}</blockquote>}</section>
