...
export default async function Categoria({ params }: { params: Promise<{ id: string }> }) {
  ...
  const category = await prisma.category.findFirst({
    ...
    include: {
      ...
      products: {
        include: {
          offers: { where: { active: true }, include: { supplier: true } },
  ...
  const spend = category.products
    .flatMap(({ requisitionLines }) => requisitionLines)
  ...
  const supplierIds = new Set(
    category.products.flatMap(({ offers }) => offers.map(({ supplierId }) => supplierId)),
  );
  const opportunities = category.products
    .map((product) => {
      const prices = product.offers.map((offer) =>
        effectiveCost(
          offer.normalizedUnitPrice ?? offer.unitPrice,
          offer.taxRate,
          context.organization.vatDeductibilityPercent,
        ),
      );
      return {
        product,
  ...
  return (
    <main>
      <PageHeader
        eyebrow="Categoria 360"
        title={category.name}
        description="Decisioni, domanda e copertura fornitori in un’unica lettura."
      />
      <section className="decision-signals">
        <header>
          <p className="eyebrow">Cosa fare</p>
          <h2>Priorità della categoria</h2>
        </header>
        <div>
          ...
          <Link href="#fornitori">
            <strong>{averageCoverage.toFixed(1)}</strong>
            <span>fornitori medi per prodotto</span>
      ...
      <div className="metrics-grid four">
        <Metric label="Spesa osservata" value={formatMoney(spend)} />
        <Metric label="Budget" value={formatMoney(budget)} />
        <Metric label="Prodotti attivi" value={category.products.length} />
        <Metric label="Fornitori" value={supplierIds.size} />
      </div>
      <div className="dashboard-columns">
        <section id="opportunita">
          <div className="section-heading">
            <div>
              <p className="eyebrow">Prezzi</p>
              <h2>Prodotti con confronto utile</h2>
            </div>
            <span>Top 5</span>
          </div>
          {opportunities.slice(0, 5).map(({ product, spread }) => (
            <Link
              className="activity-row"
              key={product.id}
              href={`/products/${product.id}#offerte`}
            >
              <div>
                <strong>{product.name}</strong>
                <span>
                  {product.offers.length} offerte dello stesso prodotto · confronta condizioni e
                  fonti
                </span>
              </div>
        ...
        <section id="fornitori">
          <div className="section-heading">
            <div>
              ...
              <h2>Fornitori della categoria</h2>
            </div>
            <span>{supplierIds.size} attivi</span>
          </div>
          {[...supplierIds].slice(0, 5).map((supplierId) => {
            const offer = offers.find((item) => item.supplierId === supplierId)!;
            const covered = category.products.filter((product) =>
              product.offers.some((item) => item.supplierId === supplierId),
            ).length;
            return (
              <Link
                className="activity-row"
                key={supplierId}
                href={`/suppliers/${supplierId}?tab=products`}
              >
                <div>
                  <strong>{offer.supplier.name}</strong>
                  <span>
                    {covered} prodotti · minimo{" "}
                    {offer.supplier.minimumOrderValue
                      ? formatMoney(Number(offer.supplier.minimumOrderValue))
                      : "n.d."}{" "}
                    · franco{" "}
                ...
                <b>{((covered / Math.max(category.products.length, 1)) * 100).toFixed(0)}%</b>
              </Link>
            );
          })}
        </section>
