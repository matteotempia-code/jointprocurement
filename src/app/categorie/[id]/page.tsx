...
export default async function Categoria({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main><PageHeader eyebrow="Categoria 360" title={category.name} description="Decisioni, domanda e copertura fornitori in un’unica lettura." />
    ...
    <div className="dashboard-columns"><section id="opportunita"><div className="section-heading"><div><p className="eyebrow">Prezzi</p><h2>Prime opportunità</h2></div><span>Top 5</span></div>{opportunities.slice(0, 5).map(({ product, spread }) => <Link className="activity-row" key={product.id} href={`/products/${product.id}`}><div><strong>{product.name}</strong><span>{product.offers.length} offerte confrontabili</span></div><b>{spread.toFixed(1)}%</b></Link>)}</section><section id="fornitori"><div className="section-heading"><div><p className="eyebrow">Copertura</p><h2>Concentrazione fornitori</h2></div><span>{supplierIds.size} attivi</span></div>{[...supplierIds].slice(0, 5).map((supplierId) => { const offer = offers.find((item) => item.supplierId === supplierId)!; const covered = category.products.filter((product) => product.offers.some((item) => item.supplierId === supplierId)).length; return <Link className="activity-row" key={supplierId} href={`/suppliers/${supplierId}`}><div><strong>{offer.supplier.name}</strong><span>{covered} prodotti coperti</span></div><b>{(covered / Math.max(category.products.length, 1) * 100).toFixed(0)}%</b></Link>; })}</section></div>
  </main>;
}
