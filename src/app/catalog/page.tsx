...
export default async function Catalogo({ searchParams }: Props) {
  ...
  const searchClauses = searchTerms.map((term) => ({
    OR: (term.toLocaleLowerCase("it-IT") === "guanti" ? [term, "guanto"] : [term]).flatMap(
      (value) => [
        ...
        { manufacturerSku: { contains: value, mode: "insensitive" as const } },
      ],
    ),
  }));
  const where = {
    ...
    AND: [
      ...
      filters.favorite === "true"
        ? { favorites: { some: { userId: context.user.id, facilityId: scope.id } } }
        : {},
    ],
  };
  ...
  const rows = products
    .map((product) => ({
      product,
      offer:
        getPreferredOffer(product.offers) ??
        [...product.offers].sort((a, b) => getComparablePrice(a) - getComparablePrice(b))[0],
    }))
    .sort((a, b) =>
      filters.sort === "price"
        ? getComparablePrice(a.offer) - getComparablePrice(b.offer)
        : filters.sort === "lead"
          ? Number(a.offer?.leadTimeDays ?? 99) - Number(b.offer?.leadTimeDays ?? 99)
  ...
  const active = [
    ["q", filters.q],
    ["category", categories.find((item) => item.id === filters.category)?.name],
    ["supplier", suppliers.find((item) => item.id === filters.supplier)?.name],
    ["preferred", filters.preferred === "true" ? "Solo convenzionati" : undefined],
  ] as const;
  return (
    <main className="phase1-page phase1-catalog">
      <PageHeader
        eyebrow="Acquisto guidato"
        title="Catalogo"
        description="Cerca, confronta il costo normalizzato e scegli la quantità."
      />
      <nav className="catalog-tabs">
        <Link href="/catalog">Tutti</Link>
        <Link href="/catalog?favorite=true">Preferiti</Link>
        <Link href="/liste">Liste ricorrenti</Link>
        <Link href="/richieste#fuori-catalogo">Fuori catalogo</Link>
      </nav>
      <form className="filter-bar catalog-filters">
        ...
        <select name="sort" defaultValue={filters.sort ?? "name"}>
          <option value="name">Rilevanza</option>
          <option value="price">Prezzo normalizzato</option>
          <option value="lead">Consegna più rapida</option>
        </select>
        <label className="checkbox">
          <input
            ...
            name="preferred"
            value="true"
            defaultChecked={filters.preferred === "true"}
          />{" "}
          Convenzionati
        </label>
        <button className="primary-cta">Applica</button>
      </form>
      {active.some(([, value]) => value) && (
        <div className="active-filter-row">
          {active.map(
            ([key, value]) =>
              value && (
                <Link key={key} href={query({ [key]: undefined, page: "1" })}>
      ...
      <div className="catalog-result-head">
        <p>
          <strong>{totalCount}</strong> {totalCount === 1 ? "prodotto" : "prodotti"} · pagina{" "}
          {Math.min(page, pageCount)} di {pageCount}
        </p>
        <span>Prezzi IVA esclusa · listino attivo</span>
      </div>
      {rows.length ? (
        <div className="refined-catalog">
              ...
              <article key={product.id}>
                <div className="product-identity">
                  ...
                  <div>
                    ...
                    <p>
                      {product.brand} · {product.packageDescription}
                    </p>
                  </div>
                </div>
                <div className="catalog-commercial">
                  {offer && normalized ? (
                    <PriceBlock
                      normalizedPrice={normalized.normalizedPrice}
                      normalizedUom={product.consumptionUomLabel}
                      packPrice={Number(offer.unitPrice)}
                      packSize={product.packageDescription}
                      variant="table"
                    />
                  ) : (
                    <strong>Non confrontabile</strong>
                  )}
                  <div className="catalog-supplier">
                    <strong>{offer?.supplier.name}</strong>
                    <span>
                      {offer?.preferred ? "Convenzionato" : "Listino attivo"} · consegna{" "}
                      {offer?.leadTimeDays} gg
                    </span>
                  </div>
                  <StatusChip variant={offer?.availabilityStatus === "IN_STOCK" ? "ok" : "warn"}>
                  ...
                  <footer className="catalog-actions">
                    <form action={toggleFavorite}>
                      ...
                      <button className="favorite-button">
                        {favoriteIds.has(product.id) ? "Salvato" : "Preferito"}
                      </button>
                    </form>
                    <ProductActionsMenu
                      ...
                      compareHref={`/products/${product.id}#offerte`}
                    />
                    {offer && (
                      <form action={addToCart} className="add-control">
                        ...
                        <input
                          ...
                          min="1"
                          defaultValue="1"
                          aria-label={`Quantità di ${product.name}`}
                        />
                        <button className="secondary-cta">Aggiungi</button>
                      </form>
                    )}
                  </footer>
                )}
        ...
        <EmptyState
          title="Nessun prodotto corrisponde alla ricerca"
          description="Modifica i filtri oppure invia una richiesta fuori catalogo."
          action={
            <Link className="primary-cta" href="/richieste#fuori-catalogo">
      ...
      {pageCount > 1 && (
        <nav className="phase1-pagination" aria-label="Paginazione catalogo">
          <Link aria-disabled={page <= 1} href={query({ page: String(Math.max(1, page - 1)) })}>
            Precedente
          </Link>
          <span>
            {page} / {pageCount}
          ...
          <Link
            ...
            href={query({ page: String(Math.min(pageCount, page + 1)) })}
          >
            Successiva
          </Link>
        </nav>
