...
export default async function ControlTower() {
  ...
  const [orders, offers, issues, products, organizations] = await Promise.all([
    ...
    prisma.qualityIssue.count({
      where: { purchaseOrderLine: { purchaseOrder: { organizationId } }, status: "OPEN" },
    }),
  ...
  const spend = orders
    .filter((order) => order.issuedAt.getFullYear() === new Date().getFullYear())
  ...
  const opportunities = products
    ...
    .map((product) => ({
      product,
      comparison: compareOffers(product.offers, context.organization.vatDeductibilityPercent),
    }))
    .filter(({ comparison }) => comparison.spread > 5)
    .sort((a, b) => b.comparison.spread - a.comparison.spread);
  const observedOpportunity = opportunities.reduce((sum, { product, comparison }) => {
    const observedQuantity = orders
      .flatMap((order) => order.lines)
      .filter((line) => line.canonicalProductId === product.id)
    ...
    return (
      ...
      Math.max(
        0,
        Number(preferred?.unitPrice ?? comparison.lowest?.unitPrice ?? 0) -
          Number(comparison.lowest?.unitPrice ?? 0),
      ) *
        observedQuantity
    );
  }, 0);
  ...
  const organizationSpend = organizations.map((organization) => ({
    ...organization,
    spend: orders
    ...
    <main>
      ...
      <div className="executive-brief">
        <section>
          ...
          {opportunities.slice(0, 3).map(({ product, comparison }, index) => (
        ...
        <section>
          <div className="section-heading">
            <div>
              <p className="eyebrow">Top 3 rischi</p>
              <h2>Cosa presidiare</h2>
            </div>
          </div>
          <article>
            <span>01</span>
            <div>
              ...
              <small>Ordini da sollecitare</small>
            </div>
            <b>{overdue}</b>
          </article>
          <article>
      ...
      <section>
        <div className="section-heading">
          <div>
            <p className="eyebrow">Rete congiunta</p>
            <h2>Anteo e Coopselios</h2>
          </div>
          <span>Spesa osservata, non annualizzata</span>
        </div>
        <div className="organization-comparison">
          {organizationSpend.map((organization) => (
            <div key={organization.id}>
              <span>{organization.name}</span>
              <strong>{formatMoney(organization.spend)}</strong>
              <i>
                <b
                  style={{
                    width: `${(organization.spend / Math.max(...organizationSpend.map((item) => item.spend), 1)) * 100}%`,
                  }}
                />
              </i>
            </div>
