...
export default async function ImportDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ filtro?: string; q?: string; ordine?: string; pagina?: string; eccezione?: string; caricato?: string; pubblicato?: string; batch?: string }> }) {
  ...
  const summary = (job.summary ?? {}) as { duplicateDocumentId?: string | null };
  ...
  return <main>
    ...
    {!job.sourceDocument.supplierId && <form action={confirmImportSupplier} className="supplier-confirmation"><input type="hidden" name="jobId" value={job.id} /><div><p className="eyebrow">Conferma necessaria</p><h2>A quale fornitore appartiene il documento?</h2><p>Nessun fornitore viene creato automaticamente.</p></div><label>Fornitore<select name="supplierId" required defaultValue=""><option value="" disabled>Seleziona un fornitore</option>{suppliers.map((supplier) => <option key={supplier.id} value={supplier.id}>{supplier.name}</option>)}</select></label><button className="primary-cta">Conferma fornitore</button></form>}
    {job.status === "FAILED" && <section className="import-failure" role="alert"><p className="eyebrow">Elaborazione interrotta</p><h2>Non siamo riusciti a identificare dati pubblicabili</h2><p>{job.errorMessage ?? "Errore non identificato."}</p></section>}
