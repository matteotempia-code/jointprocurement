...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const source = await prisma.sourceDocument.findFirst({
    where: { id: (await params).id, organizationId: context.assignment.organizationId },
  });
  ...
  try {
    ...
    return new NextResponse("Il file originale non e disponibile nello storage configurato.", {
      status: 404,
    });
  }
}
