...
export type DocumentIntelligence = {
  supplierCandidate: EvidenceValue<string>;
  supplierVatNumber: EvidenceValue<string>;
  priceListTitle: EvidenceValue<string>;
  currency: EvidenceValue<string>;
  issueDate: EvidenceValue<string>;
  validFrom: EvidenceValue<string>;
  validUntil: EvidenceValue<string>;
...
export interface ProcurementAIProvider {
  id: "OPENAI" | "LOCAL_HEURISTIC" | "ANTHROPIC";
  model: string | null;
  isAi: boolean;
  ...
  interpretCommercialConditions(
    text: string,
    context: AIContext,
  ): Promise<DocumentIntelligence["commercialConditions"]>;
  interpretProductRow(
    raw: string,
  ...
  interpretTechnicalDocument(
    filename: string,
    text: string,
    context: AIContext,
  ): Promise<TechnicalInterpretation | null>;
}
