...
export function openIssuesWhere(
  scope: FacilityScope,
): import("@prisma/client").Prisma.QualityIssueWhereInput {
  return {
    purchaseOrderLine: { purchaseOrder: { facilityId: { in: scope.facilityIds } } },
...
export async function getPreferredCompliance(scope: FacilityScope) {
  const lines = await prisma.purchaseRequisitionLine.findMany({
    where: { requisition: { facilityId: { in: scope.facilityIds }, status: "APPROVED" } },
    select: { quantity: true, supplierOffer: { select: { preferred: true } } },
  });
  const total = lines.reduce((sum, line) => sum + Number(line.quantity), 0);
  const preferred = lines
    .filter((line) => line.supplierOffer.preferred)
