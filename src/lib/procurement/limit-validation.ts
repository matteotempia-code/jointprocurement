...
export function validateProcurementLimit(input: ProcurementLimitConfiguration) {
  ...
  if (input.limitType === "MONETARY") {
    ...
    return { maximumAmount: positiveNumber(input.maximumAmount, "L'importo massimo"), maximumQuantity: null };
  }
  ...
  return { maximumAmount: null, maximumQuantity: positiveNumber(input.maximumQuantity, "La quantità massima") };
}
