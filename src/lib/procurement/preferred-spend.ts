...
export function preferredSpendShare(
  lines: SpendLine[],
  preferredSupplierProducts: Iterable<string>,
) {
  ...
  const totals = lines.reduce(
    (result, line) => {
      ...
      result.total += amount;
      if (preferred.has(`${line.supplierId}:${line.canonicalProductId}`))
        result.preferred += amount;
      return result;
    },
    { total: 0, preferred: 0 },
  );
