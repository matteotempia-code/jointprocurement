...
export async function getFacilityBudget(facilityId: string) {
  ...
  const [orders, pending] = await Promise.all([
    prisma.purchaseOrder.aggregate({
      ...
      where: {
        ...
        issuedAt: { gte: new Date(current.getFullYear(), 0, 1) },
      },
    }),
  ...
  const approved = budgets.reduce((s, b) => s + Number(b.approvedAmount), 0),
    actual = budgets.reduce((s, b) => s + Number(b.actualAmount), 0),
    committed = Number(orders._sum.total ?? 0),
    reserved = Number(pending._sum.total ?? 0);
