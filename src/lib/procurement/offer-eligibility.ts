...
export function offerAvailability(offer: PurchasableOffer, at = new Date()) {
  ...
  if (offer.validFrom && offer.validFrom > at)
    return { purchasable: false, reason: "OFFER_NOT_STARTED" as const };
  if (offer.validUntil && offer.validUntil < at)
    return { purchasable: false, reason: "OFFER_EXPIRED" as const };
