...
export function effectiveCost(
  ...
  if (
    ![net, vat, deductible].every(Number.isFinite) ||
    net < 0 ||
    vat < 0 ||
    deductible < 0 ||
    deductible > 100
...
export function getComparablePrice(
  offer: NumericOffer,
  vatDeductibilityPercent: unknown = 100,
): number {
...
export function compareOffers<T extends NumericOffer>(
  offers: T[],
  vatDeductibilityPercent: unknown = 100,
) {
  const sorted = [...offers].sort(
    (a, b) =>
      getComparablePrice(a, vatDeductibilityPercent) -
      getComparablePrice(b, vatDeductibilityPercent),
  );
  ...
  const preferredDelta =
    preferred && low > 0
      ? ((getComparablePrice(preferred, vatDeductibilityPercent) - low) / low) * 100
      : 0;
