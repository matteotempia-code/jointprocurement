...
const aliases: Record<ImportField, string[]> = {
  ...
  category: ["categoria", "famiglia", "categoria merceologica"],
  subcategory: ["sottocategoria", "sotto categoria"],
  purchaseUom: ["um", "udm", "unità acquisto", "unita acquisto", "unit", "uom"],
  packageDescription: ["confezione", "formato", "pack description", "pack", "contenuto"],
  unitsPerPackage: [
    "pz/conf",
    "pezzi confezione",
    "unità confezione",
  ...
  grossPrice: ["prezzo lordo", "listino", "gross price"],
  discount: ["sconto", "discount"],
  netPrice: ["prezzo netto", "prezzo unitario", "unit price", "prezzo", "net price", "price"],
  taxRate: ["iva", "aliquota iva", "vat"],
  currency: ["valuta", "currency"],
  moq: ["moq", "ordine minimo", "minimo ordine"],
  validFrom: ["valido dal", "validità da", "data inizio"],
  validUntil: ["valido al", "validità a", "data fine"],
  leadTimeDays: ["consegna giorni", "lead time", "giorni consegna"],
...
export function suggestColumnMapping(headers: string[]) {
  ...
  for (const header of headers) {
    ...
    if (best && best.score >= 0.8 && !Object.values(result).includes(best.field))
...
export function applyColumnMapping(
  ...
  for (const [source, target] of Object.entries(mapping)) {
    ...
    if (value !== undefined && value !== null && String(value).trim() !== "")
      interpreted[target] = typeof value === "number" ? value : String(value).trim();
  }
  return interpreted;
}
