...
function extension(filename: string) {
  return filename.toLocaleLowerCase("it-IT").split(".").pop() ?? "";
}
...
class XlsxRuntimeDiagnosticError extends Error {
  constructor(
    public readonly diagnostic: XlsxRuntimeDiagnostic,
    cause: unknown,
  ) {
    super(`${diagnostic.marker}: ${diagnostic.errorMessage ?? "errore XLSX non identificato"}`, {
      cause,
    });
...
export async function parseSpreadsheet(
  buffer: Buffer,
  expected?: { byteLength?: number; checksum?: string },
  ...
  try {
    if (!diagnostic.zipSignature)
      throw new Error(
        "Il file XLSX non è valido o è incompleto: archivio workbook non riconosciuto.",
      );
    ...
    if (!diagnostic.moduleDefaultExists)
      throw new Error("Il parser XLSX non è disponibile nel runtime server.");
    ...
    diagnostic.worksheetNames = Array.isArray(worksheets)
      ? worksheets.map((sheet) => sheet.sheet).slice(0, 30)
      : [];
    if (!Array.isArray(worksheets) || worksheets.length === 0)
      throw new Error("Il file XLSX non contiene fogli di lavoro leggibili.");
    ...
    for (const worksheet of worksheets) {
      ...
      const headers =
        matrix[headerIndex]?.map((value, index) =>
          String(value || `Colonna ${index + 1}`).trim(),
        ) ?? [];
      const rows = matrix
        .slice(headerIndex + 1)
        ...
        .map((values, index) => {
          ...
          return {
            values: mapped,
            rawSource: values.map((value) => String(value ?? "")).join(" | "),
            locator: { sheet: worksheet.sheet, row: headerIndex + index + 2 },
          };
        });
      ...
      if (candidate && score > selectedScore) {
        selectedScore = score;
        selectedRows = rows;
        selectedPreview = matrix
          .slice(0, 20)
          .map((values) => values.map(String).join(" | "))
          .join("\n");
        sheets.forEach((sheet) => {
          sheet.selected = sheet.name === worksheet.sheet;
        });
      }
    }
    if (!selectedRows.length)
      throw new Error("Non è stata identificata una tabella prodotti nel workbook.");
    return {
      parserType: "XLSX_DETERMINISTIC",
      sheets,
      rows: selectedRows,
      textPreview: selectedPreview.slice(0, 5000),
...
export function parseDelimited(text: string): ParsedDocument {
  ...
  const delimiter = candidates.sort(
    (a, b) =>
      parseDelimitedLine(lines[0] ?? "", b).length - parseDelimitedLine(lines[0] ?? "", a).length,
  )[0];
  let headerIndex = 0;
  let best = 0;
  for (let index = 0; index < Math.min(lines.length, 20); index += 1) {
    const score = knownHeaderScore(parseDelimitedLine(lines[index], delimiter));
    if (score > best) {
      best = score;
      headerIndex = index;
    }
  ...
  const rows = lines.slice(headerIndex + 1).map((line, index) => ({
    values: Object.fromEntries(
      headers.map((header, column) => [header, parseDelimitedLine(line, delimiter)[column] ?? ""]),
    ),
    rawSource: line,
    locator: { row: headerIndex + index + 2 },
  }));
  return {
    parserType: "CSV_DETERMINISTIC",
    sheets: [{ name: "Dati", records: rows.length, selected: true }],
    rows,
  };
}
...
function parseTextTable(text: string, parserType: string): ParsedDocument {
  ...
  if (tableLines.length >= 2) {
    const parsed = parseDelimited(tableLines.join("\n"));
    return {
      ...parsed,
      parserType,
      ...
      rows: parsed.rows.map((row) => ({
        ...row,
        locator: { ...row.locator, ...(parserType.startsWith("PDF") ? { page: 1 } : { table: 1 }) },
      })),
    };
  }
...
export async function parseDocument(
  ...
  if (!supportedExtensions.has(ext))
    throw new Error("Formato non supportato. Usa XLSX, CSV, TSV, PDF, DOCX, TXT o un’immagine.");
  if (ext === "xls")
    throw new Error(
      "Il formato XLS binario legacy è stato conservato, ma il parser locale non può leggerlo in sicurezza. Salvalo come XLSX o CSV e riprova.",
    );
  if (["png", "jpg", "jpeg"].includes(ext))
    throw new Error(
      "Interpretazione automatica delle immagini non disponibile in questo ambiente. Il documento è stato conservato per una futura elaborazione OCR.",
    );
  ...
  if (ext === "docx") {
    ...
    return parseTextTable(result.value, "DOCX_TEXT_DETERMINISTIC");
  }
  if (ext === "pdf") {
    const { extractPdfText } = await import("./pdf-parser");
    const text = await extractPdfText(buffer);
    if (!text.trim())
      throw new Error("PDF senza testo estraibile. OCR non disponibile in questo ambiente.");
