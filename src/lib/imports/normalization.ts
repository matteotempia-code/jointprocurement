...
const normalizeUom = (value: unknown) =>
  uomAliases[
    String(value ?? "")
      .trim()
...
function inferConsumptionUom(input: InterpretedFields, purchaseUom: string) {
  ...
  const isLiquidProduct =
    /acqua|ammorbident|bevanda|brillantant|candegg|detergent|disinfett|latte|liquid|olio|sapone|sgrass|shampoo|soluzion|succo/.test(
      description,
    );
...
export function normalizeImportedFields(input: InterpretedFields): NormalizedImport {
  ...
  const packageUnits = String(input.packageDescription ?? "").match(
    /(?:cf|conf|box|cartone|pack)?\s*[x×:]?\s*(\d+(?:[.,]\d+)?)(?:\s*(?:pz|pezzi|unità|unita))?/i,
  )?.[1];
  ...
  const net =
    explicitNet ?? (gross !== null && discount !== null ? gross * (1 - discount / 100) : gross);
  ...
  if (discount !== null && (discount < 0 || discount > 100))
    errors.push("Sconto fuori intervallo 0–100%");
  ...
  if (validFrom && validUntil && validFrom > validUntil)
    errors.push("La validità termina prima della data di inizio");
  if (input.currency && String(input.currency).toLocaleUpperCase("it-IT") !== "EUR")
    warnings.push("Valuta differente da EUR da verificare");
  if (taxRate !== null && ![0, 4, 5, 10, 22].includes(taxRate))
    warnings.push("Aliquota IVA non standard da verificare");
  ...
  const normalized = normalizeOfferPrice(
    {
      ...
      consumptionUom: consumptionUom || null,
      consumptionUomLabel:
        consumptionUom === "PIECE"
          ...
          : consumptionUom === "L"
            ...
            : consumptionUom === "KG"
              ? "kg"
              : consumptionUom.toLocaleLowerCase("it-IT"),
      packageDescription: input.packageDescription ? String(input.packageDescription) : null,
    },
    { unitPrice: net ?? -1, packageSize: units, packageUnit: purchaseUom },
  );
  ...
  return {
    ...
    moq,
    unitsPerPackage: units,
    leadTimeDays: parseItalianNumber(input.leadTimeDays),
    validFrom,
    validUntil,
    purchaseUom,
    consumptionUom,
