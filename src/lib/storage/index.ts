...
export function configuredDocumentStorageProvider() {
  ...
  throw new Error(
    "DOCUMENT_STORAGE_PROVIDER deve essere configurato esplicitamente come 'supabase' o 'local'.",
  );
}

export function getDocumentStorage(
  provider = configuredDocumentStorageProvider(),
...
export function locatorForNewDocument(objectKey: string): DocumentStorageLocator {
  ...
  return {
    provider,
    bucket: provider === "supabase" ? configuredSupabaseBucket() : null,
    objectKey,
  };
}

export function locatorFromSourceDocument(
  source: Pick<
    SourceDocument,
    "storageProvider" | "storageBucket" | "storageObjectKey" | "storagePath"
  >,
): DocumentStorageLocator {
  if (source.storageProvider === "supabase" && source.storageBucket && source.storageObjectKey)
    return {
      provider: "supabase",
  ...
  if (source.storageProvider === "local") {
    ...
    const normalized =
      ...
      (legacy.startsWith("demo-imports/")
        ? `fixtures/${legacy.slice("demo-imports/".length)}`
        : legacy.replace(/^var\/imports\//, ""));
