...
export function configuredSupabaseBucket() {
  return required("SUPABASE_STORAGE_BUCKET");
}
...
export class SupabaseDocumentStorage implements DocumentStorageProvider {
  ...
  constructor(options: { url?: string; serviceKey?: string; bucket?: string } = {}) {
    ...
    if (!/^[a-z0-9][a-z0-9._-]{1,62}$/.test(this.bucket))
      throw new Error("SUPABASE_STORAGE_BUCKET non valido.");
  ...
  private key(locator: DocumentStorageLocator) {
    if (locator.provider !== this.id || locator.bucket !== this.bucket)
      throw new Error("Locator Supabase non valido.");
  ...
  async put(locator: DocumentStorageLocator, data: Buffer, contentType: string) {
    const { error } = await this.client.storage
      .from(this.bucket)
      .upload(this.key(locator), data, { contentType, upsert: false });
  ...
  async get(locator: DocumentStorageLocator) {
    ...
    if (error || !data)
      throw new Error(
        `Documento non disponibile in Supabase Storage${error ? `: ${error.message}` : "."}`,
      );
    return Buffer.from(await data.arrayBuffer());
  }

  async exists(locator: DocumentStorageLocator) {
    return (await this.head(locator)) !== null;
  }
  ...
  async head(locator: DocumentStorageLocator) {
    ...
    const { data, error } = await this.client.storage
      .from(this.bucket)
      .list(key.slice(0, slash), { search: key.slice(slash + 1), limit: 2 });
    ...
    return {
      ...
      contentType: typeof item.metadata?.mimetype === "string" ? item.metadata.mimetype : undefined,
    };
  }
  ...
  async createSignedUrl(locator: DocumentStorageLocator, expiresInSeconds: number) {
    ...
    const { data, error } = await this.client.storage
      .from(this.bucket)
      .createSignedUrl(this.key(locator), ttl);
    if (error || !data?.signedUrl)
      throw new Error(`URL firmato non disponibile${error ? `: ${error.message}` : "."}`);
  ...
  async createPrivateBucket() {
    return this.client.storage.createBucket(this.bucket, {
      public: false,
