...
function safeSegment(value: string, label: string) {
  if (!value || !/^[a-zA-Z0-9_-]+$/.test(value))
    throw new Error(`${label} non valido per lo storage.`);
  return value;
}
...
export function buildDocumentObjectKey(input: {
  ...
  if (!/^[a-f0-9]{64}$/.test(input.checksum))
    throw new Error("Checksum non valido per lo storage.");
...
export function buildOperationalAttachmentKey(input: {
  organizationId: string;
  ownerType: string;
  ownerId: string;
  attachmentId: string;
...
export function buildTechnicalDocumentKey(input: {
  organizationId: string;
  sourceDocumentId: string;
  checksum: string;
  filename: string;
