  ...
  return (
    <details className="product-actions-menu">
      ...
      <div className="product-actions-popover">
        ...
        {lists.length ? (
          <div className="list-choice">
            {lists.slice(0, 4).map((list) => (
              <form action={addProductToList} key={list.id}>
        ...
        <details className="create-list-choice">
          <summary>+ Crea nuova lista</summary>
          <form action={createShoppingList}>
            ...
            <input
              ...
              aria-label={`Nome della lista per ${productName}`}
            />
            <button>Crea e aggiungi</button>
          </form>
        </details>
        <nav aria-label="Approfondimenti prodotto">
          <Link href={detailHref}>Apri dettagli</Link>
          {compareHref && <Link href={compareHref}>Confronta offerte</Link>}
        </nav>
