        ...
        : status === "NEEDS_REVIEW"
          ...
          : status === "FAILED"
            ...
            : ["PARSING", "PARSED"].includes(status)
              ? 2
              : ["INTERPRETING", "INTERPRETED"].includes(status)
                ? 3
                : 1;
  return (
    <ol className="import-progress" aria-label="Avanzamento importazione">
...
export function Confidence({
  ...
  return (
    <span
      className={`confidence ${confidenceClass(value)}`}
      title={value == null ? undefined : `${Math.round(value * 100)}%`}
    >
      <i />
      {detail ?? confidenceLabel(value)}
    </span>
  );
}
...
export function ImportStatus({ status }: { status: string }) {
  return (
    <span className={`import-status import-status-${status.toLocaleLowerCase("it-IT")}`}>
      <i />
...
export function ImportTabs({ jobId, active = "preview" }: { jobId: string; active?: string }) {
  ...
  return (
    <nav className="import-tabs" aria-label="Sezioni importazione">
