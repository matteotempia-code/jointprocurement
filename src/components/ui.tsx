...
export function StatusIndicator({ active, label }: { active: boolean; label?: string }) {
  return (
    <StatusChip variant={active ? "ok" : "neutral"}>
...
export function InlineMeta({
  ...
  return (
    <span className={`inline-meta ${className}`.trim()}>
      {visible.map((item, index) => (
        <span key={index}>
          {index > 0 && <i aria-hidden>{separator}</i>}
          <span>{item}</span>
        </span>
      ))}
    </span>
...
export function Num({
  ...
  return (
    <span className={`num ${className}`.trim()}>
      {new Intl.NumberFormat("it-IT", options).format(kind === "percent" ? value / 100 : value)}
    </span>
  );
}
...
export function PriceBlock({
  ...
  variant?: "default" | "compact" | "table" | "mobile";
}) {
  return (
    <span className={`price-block price-block-${variant}`}>
      <strong>
        {normalizedPrice == null ? (
          "Non confrontabile"
        ) : (
          <>
            <Num value={normalizedPrice} kind="currency" digits={4} />
            {normalizedUom && ` / ${normalizedUom}`}
          </>
        )}
      </strong>
      <small>
        <Num value={packPrice} kind="currency" />
        {packSize ? ` · ${packSize}` : " · confezione"}
      </small>
    </span>
  );
}
...
export function ScopeBadge({ type, label }: { type: string; label: string }) {
  const typeLabel =
    (
      { ORGANIZATION: "Organizzazione", AREA: "Area", FACILITY: "Struttura" } as Record<
        string,
        string
      >
    )[type] ?? type;
...
export function Pagination({
  ...
  const href = (next: number) => {
    ...
    query.set("pagina", String(next));
    return `${pathname}?${query}`;
  };
  return (
    <nav className="phase1-pagination" aria-label="Paginazione">
      <Link
...
export function FutureButton({ children }: { children: ReactNode }) {
  return (
    <div className="future-action">
      ...
      <span>Disponibile nella prossima fase</span>
    </div>
  );
}
