...
export function ImportUploadForm({
  ...
  return (
    <form action={action} className="import-upload-form">
      <label className="drop-zone">
        ...
        <span className="import-file-picker">
          <b>Scegli file</b>
          <span>{filename}</span>
        </span>
        <input
          ...
          name="file"
          type="file"
          accept=".xlsx,.xls,.csv,.tsv,.pdf,.docx,.txt,.png,.jpg,.jpeg"
          onChange={(event) =>
            setFilename(event.currentTarget.files?.[0]?.name ?? "Nessun file selezionato")
          }
        />
      </label>
      ...
      <label>
        ...
        <textarea
          name="notes"
          placeholder="Contesto o indicazioni utili per chi verificherà i dati"
        />
      </label>
      {state.error && (
      ...
      <div className="import-honesty-note">
        <strong>Come verrà elaborato</strong>
        <span>{processingExplanation}</span>
      </div>
      <button className="primary-cta" disabled={pending}>
        {pending ? "Caricamento e lettura…" : "Carica e interpreta"}
      </button>
    </form>
  );
}
